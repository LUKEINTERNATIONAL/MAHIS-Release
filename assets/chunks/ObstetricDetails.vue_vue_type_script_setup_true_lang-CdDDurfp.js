import { bW as getAgeInDays, n as icons, r as StandardModal, z as StandardForm, F as DynamicButton, t as toastWarning, K as ObservationService, b as EncounterTypeId, G as toastSuccess, _ as _export_sfc, u as useDemographicsStore, b5 as RelationshipService, C as useExposeFromStandardForm, a1 as modifyFieldValue, ar as ConceptService, H as HisDate, a2 as getFieldValue } from '../index-D9jmStal.js';
import { c as computed, eF as create$3, eG as create$5, eH as create$6, s as defineComponent, w as watch, y as openBlock, O as createBlock, B as withCtx, A as createVNode, F as unref, a$ as personOutline, C as createBaseVNode, H as createCommentVNode, bF as IonModal, f as ref, a2 as onMounted, n as nextTick, x as resolveComponent, z as createElementBlock, a5 as createTextVNode, J as Fragment, R as renderList, a4 as normalizeClass, D as toDisplayString } from './vendor-DpSS1aB1.js';
import { F as FindRegisterPatient } from './FindRegisterPatient-Zr7KMCsE.js';
import { d as defineStore, s as storeToRefs } from './pinia-CbGjwPb6.js';
import { l as lodashExports } from './lodash-D-_KzUJJ.js';
import { V as Validation } from './StandardValidations-BHq0QUIE.js';
...
const BabyDetailsModal = /* @__PURE__ */ _export_sfc(_sfc_main$3, [["__scopeId", "data-v-3a16312c"]]);
...
const _sfc_main$2 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    const getObsValueByConceptName = async (obsArray, conceptName) => {
      const obsList = obsArray.filter((o) => o.concept_name?.toLowerCase() === conceptName.toLowerCase());
      if (obsList.length === 0) return null;
      const values = [];
      for (const o of obsList) {
        const v = o.value_text ?? o.value_numeric ?? o.value_datetime ?? o.value_date;
        if (v != null && v !== "") {
          values.push(v);
        } else if (o.value_coded != null && o.value_coded !== "") {
          const name = await ConceptService.getConceptName(Number(o.value_coded));
          if (name) values.push(name);
        }
      }
      return values.length ? values : null;
    };
    const getFirstObs = async (obsArray, conceptName) => {
      const values = await getObsValueByConceptName(obsArray, conceptName);
    ...
    const mapLabourModeToPnc = (mode) => {
      ...
      if (m.includes("breech") || m === "br") return "Breech delivery";
      if (m.includes("caesarean") || m.includes("cesarean")) return "Caesarean section";
      if (m.includes("other")) return "Other mode of delivery";
      return null;
    };
    const mapLabourOutcomeToPnc = (condition) => {
      const c = String(condition).toLowerCase().trim();
      if (c.includes("live full term") || c.includes("live pre-term")) return { outcome: "Live births" };
      if (c.includes("neonatal death")) return { outcome: "Neonatal death" };
      if (c.includes("macerated stillbirth")) return { outcome: "Stillbirths", typeOfStillBirth: "macerated still birth" };
      if (c.includes("fresh stillbirth")) return { outcome: "Stillbirths", typeOfStillBirth: "fresh still birth" };
      return null;
    };
    ...
    const fetchAndPrefillDeliveryDetails = async () => {
      ...
      console.log("Labour", labourObs);
      if (!labourObs || labourObs.length === 0) return;
      const dateOfDelivery = await getFirstObs(labourObs, "Date of delivery");
      const timeOfDelivery = await getFirstObs(labourObs, "Time of delivery");
      const modeOfDelivery = await getFirstObs(labourObs, "Mode of delivery");
      const placeOfDelivery = await getFirstObs(labourObs, "Place of delivery");
      const staffConductingDelivery = await getFirstObs(labourObs, "Staff conducting delivery");
      const babyCondition = await getFirstObs(labourObs, "Baby general condition at birth");
      const numberOfBabies = await getFirstObs(labourObs, "Number of babies");
      if (dateOfDelivery) formRef.value.setFormValue("Date of delivery", dateOfDelivery);
      if (timeOfDelivery) {
        const timeVal = formatTimeForInput(timeOfDelivery) ?? timeOfDelivery;
        formRef.value.setFormValue("Time of delivery", timeVal);
      }
      if (modeOfDelivery) {
        const mapped = mapLabourModeToPnc(String(modeOfDelivery));
        if (mapped) formRef.value.setFormValue("Mode of delivery", mapped);
      }
      if (placeOfDelivery) {
        const mapped = mapLabourPlaceToPnc(String(placeOfDelivery));
        console.log("Mapped", placeOfDelivery, mapped);
        if (mapped) formRef.value.setFormValue("Place of delivery", mapped);
      }
      if (staffConductingDelivery) {
        const raw = String(staffConductingDelivery).trim();
        const options = ["md/co/ma/nurse midwife/cma", "pa/wa/hsa/other/none", "skilled birth attendant", "unskilled birth attendant", "other"];
        const match = options.find((opt) => raw.toLowerCase() === opt.toLowerCase());
        if (match) {
          formRef.value.setFormValue("Staff conducting delivery", match);
        } else {
          formRef.value.setFormValue("Staff conducting delivery", "other");
          formRef.value.setFormValue("otherStaff", raw);
        }
      }
      if (babyCondition) {
        const mapped = mapLabourOutcomeToPnc(String(babyCondition));
        if (mapped) {
          formRef.value.setFormValue("Outcome of the delivery", mapped.outcome);
          if (mapped.typeOfStillBirth) formRef.value.setFormValue("Type of still birth", mapped.typeOfStillBirth);
        }
      }
      if (numberOfBabies != null && numberOfBabies !== "") {
        formRef.value.setFormValue("Number of babies", String(numberOfBabies));
        const count = parseInt(String(numberOfBabies), 10);
    ...
    const hasBabyData = (baby) => {
      ...
      const info = getPersonInfo(baby);
      return !!(info.given_name || info.family_name || info.gender || info.birthdate);
    };
    ...
    const handleMainFormUpdate = (values) => {
      ...
      if (outcome !== "Live births" || !isBabyCaptureAllowed(values)) {
        babies.value = babies.value.filter((b) => b.isExisting);
        return;
      }
      updateBabiesList(count);
    };
    const updateBabiesList = (count) => {
      ...
      if (count > babies.value.length) {
        ...
        babies.value.push(...Array.from({ length: toAdd }, () => ({ isFilled: false })));
    ...
    const handleSaveBaby = (updatedData) => {
      ...
      if (babies.value[index] !== void 0) {
        babies.value[index] = { ...updatedData, isFilled: true };
      }
    };
    const closeModal = () => {
    ...
    watch(
      ...
      (newVal) => {
        if (newVal) {
          nextTick(() => fetchAndPrefillDeliveryDetails());
        }
      },
      { immediate: true }
    );
    onMounted(async () => {
      ...
      if (children?.length > 0) {
        babies.value = children.map(mapChildToBaby);
      }
    });
              ...
              default: withCtx(() => [
                ...
                babies.value.length > 0 ? (openBlock(), createElementBlock("div", _hoisted_2, [
                  createBaseVNode("div", _hoisted_3, [
                    createVNode(_component_ion_label, { class: "section-title" }, {
                      ...
                      return openBlock(), createBlock(_component_ion_card, {
                        ...
                        default: withCtx(() => [
                          createVNode(_component_ion_card_header, null, {
                            default: withCtx(() => [
                              createVNode(_component_ion_card_title, { class: "baby-title" }, {
                                default: withCtx(() => [
                          ...
                          createVNode(_component_ion_card_content, { class: "baby-content" }, {
                            default: withCtx(() => [
                              hasBabyData(baby) ? (openBlock(), createElementBlock("div", _hoisted_5, [
                                ...
                                createVNode(_component_ion_text, { class: "no-data-text" }, {
                              ...
                              createVNode(_component_ion_button, {
                                ...
                                default: withCtx(() => [
                                  createVNode(_component_ion_icon, {
                                    icon: hasBabyData(baby) ? _ctx.createOutline : _ctx.addOutline,
                                    slot: "start"
                                  }, null, 8, ["icon"]),
...
const DeliveryDetails = /* @__PURE__ */ _export_sfc(_sfc_main$2, [["__scopeId", "data-v-7292044e"]]);
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    const fetchAndPrefillObstetricDetails = async () => {
      ...
      const pastPregnanciesProblemObs = getObsValueByConceptName(ancObs, "past pregnancies complications");
      console.log("ANC", ancObs);
      if (gravidaObs) {
