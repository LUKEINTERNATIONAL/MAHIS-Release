import { w as watch, f as ref, c as computed, s as defineComponent, a3 as onMounted, y as openBlock, z as createElementBlock, C as createBaseVNode, F as unref, A as createVNode, B as withCtx, aB as IonCol, a5 as createTextVNode, af as IonRow, J as Fragment, S as renderList, O as createBlock, D as toDisplayString, N as IonButton, L as IonIcon, dj as eyeOutline, H as createCommentVNode, fC as TransitionGroup, bW as trashOutline, aS as syncOutline, ey as Transition, a8 as withModifiers, bI as addOutline, I as IonHeader, aE as IonToolbar, aF as IonTitle, bf as IonButtons, aH as IonContent, bH as IonModal, aM as useRouter, bN as useRoute, fl as onIonViewWillEnter, bw as IonPage, f2 as provide } from './vendor-DEu2hKw1.js';
import { u as useDemographicsStore, S as Service, g as getPouchDBRecords, O as OrderService, N as ConceptService, J as savePatientRecord, aZ as AppEncounterService, b as EncounterTypeId, G as toastSuccess, t as toastWarning, z as StandardForm, o as createModal, _ as _export_sfc, T as Toolbar } from '../index-Bw8adu8R.js';
import { o as neonatalInvestigationSections, N as NeonatalStepper } from './NeonatalStepper-Tw5l6DYM.js';
import { d as defineStore, s as storeToRefs } from './pinia-3T0xmcrW.js';
import { _ as _sfc_main$2 } from './EnterLabResultsModal.vue_vue_type_script_setup_true_lang-BsDlEZKC.js';
import { S as SummarySection, n as neonatalInvestigationFormKey } from './SummarySection-DRcWqRLa.js';
...
const useNeonatalInvestigationStore = defineStore("neonatalInvestigation", () => {
  ...
  const queuedTestsSummary = computed(() => {
    ...
    if (labOrders.value.length > 0) {
      const allTests = [];
      labOrders.value.forEach((order) => {
        if (order.tests && Array.isArray(order.tests)) {
          order.tests.forEach((test) => {
            const testName = test.name || test.test_name || "Unknown Test";
            const specimenName = order.specimen?.name || "Unknown Specimen";
            allTests.push(`${testName} (${specimenName})`);
          });
        }
      });
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    onIonViewWillEnter(async () => {
      stepperKey.value++;
      await investigationStore.loadLabOrders();
    });
    const investigationStore = useNeonatalInvestigationStore();
    ...
    const getSaveFnForStep = (stepIndex) => {
      ...
      if (stepIndex === finalStepIndex) {
        return async () => {
          try {
            const hasQueuedTests = investigationStore.queuedTests.length > 0;
            const hasLabOrders = investigationStore.labOrders.length > 0;
            if (hasQueuedTests) {
              await investigationStore.syncQueuedTests();
            } else if (!hasLabOrders && route.query.from !== "profile") {
              const userId = Service.getUserID() ?? -1;
            ...
            console.error("Failed to save investigation step:", error);
            throw error;
          }
        };
      }
