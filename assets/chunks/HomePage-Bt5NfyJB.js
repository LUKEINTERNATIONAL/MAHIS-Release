...
import { S as Service, u as useDemographicsStore, _ as _export_sfc, P as PatientService, t as toastWarning, a as useProgramStore, E as EncounterService, H as HisDate, g as getPouchDBRecords, b as EncounterTypeId, A as AppointmentService, c as HorizontalLine, d as _sfc_main$y, e as useGeneralStore, f as useStatusStore, h as useWindowSize, i as useEnrollementStore, R as ReportService, p as parameterizeUrl, j as STANDARD_DATE_FORMAT, k as alertConfirmation, O as OrderService, l as PreviousVitals, B as BasicInputField, m as useAdministerVaccineStore, n as icons, o as createModal, q as usePatientList, r as StandardModal, s as isPatientDeceased, v as closeVisit, w as getTodaysVisits, x as toastDanger, y as StandardValidations, z as StandardForm, C as useExposeFromStandardForm, D as PatientSearchService, F as DynamicButton, G as toastSuccess, I as alertConfirmationIonic, J as savePatientRecord, K as ObservationService, T as Toolbar, L as ProgramId, M as ConceptService, N as useWorkerStore, Q as checkAndsetSessionDate, U as resetDemographics, V as useGlobalPropertyStore, W as UserService, X as LocationService, Y as modal } from '../index-DRTgL5PD.js';
import { d as defineStore, m as mapState, s as storeToRefs } from './pinia-CoQIYQwb.js';
import { A as Appointment } from './ncd_appointment_service-jTugwsOX.js';
import { D as DateInputField } from './DateInputField-B9uBWgbh.js';
import { u as useActionButtonStore, _ as _sfc_main$G, M as ModulePicker } from './ActionButtons.vue_vue_type_script_setup_true_lang-HB94WPSG.js';
import { V as ViewToggleComponent } from './ViewToggleComponent-lndYK5fy.js';
import { B as BottomNavBar } from './bottomNavBar-Bzd4nIFI.js';
import { P as PatientReportService } from './patient_report_service-Da-MdqG5.js';
import { l as lodashExports } from './lodash-CEqBfDzm.js';
import { B as BasicForm } from './BasicForm-D0ILc38I.js';
import { B as BasicCard } from './BasicCard-Clqr7v-Y.js';
import { u as useAdministerOtherVaccineStore } from './AdministerOtherVaccinesStore-4376oH3Q.js';
import { c as customDatePicker } from './customDatePicker-BjjU7Moe.js';
import { s as saveVaccineAdministeredDrugs } from './vaccines_service-Dr6cOFHu.js';
import { g as getVaccinesData } from './dashboard_service--_joBHev.js';
import { i as img } from './Img-RarZMYTn.js';
import { R as ReusableDataTable } from './ReusableDataTable-BSexAe3K.js';
import { u as useRegistrationPagination, a as useSetRegistrationValues, _ as _sfc_main$z, b as _sfc_main$A, c as _sfc_main$B, d as _sfc_main$C, e as _sfc_main$D, f as _sfc_main$E, g as _sfc_main$F, R as RegistrationService } from './useSetRegistrationValues-D2UfJtYL.js';
import { u as useUserActivities } from './useUserActivities-D5s25jt0.js';
import { L as LandingPage } from './LandingPage-CPhb4bwJ.js';
...
const PROGRAM = ProgramId.LABOUR_AND_DELIVERY_PROGRAM;
...
async function getTotalLabourEnrollmentEncountersAllPatients() {
  const rows = await Service.getAPIOfflineJson("/encounters", { paginate: false, program_id: PROGRAM }, "labour_enrollment_encounters");
...
async function getLabourEncountersDeliveredAtThisFacilityIds() {
  return obsPatientIds("Place of delivery", "labour_place_this_facility", { valueCoded: "This facility" });
}
async function getLabourEncountersDeliveredAtHomeOrTransit() {
  return (await obsPatientIds("Place of delivery", "labour_place_home_transit", { valueCoded: ["Home", "In transit"] })).size;
}
async function getLabourEncountersWithModeCaesareanIds() {
  return obsPatientIds("Mode of delivery", "labour_mode_caesarean", { valueCoded: "Caesarean section" });
}
async function getLabourEncountersDeliveredBySkilledAttendant() {
  ...
  return (await obsPatientIds("Staff conducting delivery", "labour_staff_skilled", { valueText: text })).size;
}
async function getObstetricComplicationsRecordedPatientIds() {
  return obsPatientIds(OBSTETRIC_COMPLICATION_CONCEPT, "labour_obstetric_complications_all", {});
}
async function getObstetricComplicationBreakdownFromObs() {
  const [allRecordedIds, ...conditionSets] = await Promise.all([
    ...
    ...OBSTETRIC_COMPLICATION_VALUES.map(
      (value) => obsPatientIds(OBSTETRIC_COMPLICATION_CONCEPT, `labour_obstetric_complication_${value.replace(/\s+/g, "_")}`, { valueCoded: value })
    )
  ]);
...
async function getReferralByConditionFromObs() {
  const conditionSets = await Promise.all(
    REFERRAL_REASON_OPTIONS.filter((o) => o.label !== "Other").map(
      ({ label, valueText }) => obsPatientIds(REFERRAL_REASONS_CONCEPT, `labour_referral_reason_${valueText}`, { valueText }).then((set) => ({ label, count: set.size }))
    )
  );
...
async function getLabourDashboard() {
  ...
  const complicationBreakdown = complicationBreakdownData.complicationBreakdown.filter(({ label }) => label !== "Other").map(({ label, count }) => ({
    label,
    percentage: pct(count, totalEnrolled)
  }));
  const percentageWithObstetricComplication = pct(complicationBreakdownData.totalWithRecorded - complicationBreakdownData.noneCount, totalEnrolled);
  ...
  return {
    ...
    percentageDeliveredAtThisFacility: pct(atFacility, totalEnrolled),
    totalDeliveriesWithStaffRecorded: base.totalDeliveriesWithStaffRecorded ?? 0,
    mothersDeliveredBySkilledAttendant: skilledCount,
    percentageDeliveredBySkilledAttendants: pct(skilledCount, totalEnrolled),
    caesareanSectionCount: caesareanAtFacility,
    percentageCaesareanSection: pct(caesareanAtFacility, totalEnrolled),
...
async function getPncDashboard() {
  ...
  const programId = Service.getProgramID();
  try {
    if (Service.getLanConnectionStatus()) return emptyDashboardData();
    const raw = await Service.getJson(DASHBOARD_STATS_PATH, { program_id: programId, date });
    const payload = raw?.data && typeof raw.data === "object" ? raw.data : raw;
    if (!payload || typeof payload !== "object") return emptyDashboardData();
    return mapResponseToDashboardData(payload);
  } catch (_) {
