import { s as defineComponent, w as watch, y as openBlock, O as createBlock, F as unref, bN as IonCard, B as withCtx, A as createVNode, bc as IonCardTitle, d$ as IonSkeletonText, z as createElementBlock, L as IonIcon, aU as medicalOutline, a5 as createTextVNode, J as Fragment, bd as IonCardHeader, ap as IonList, S as renderList, aq as IonItem, a7 as IonLabel, ba as calendarOutline, C as createBaseVNode, D as toDisplayString, cK as idCardOutline, c9 as folderOutline, c6 as checkmarkCircleOutline, bf as IonCardContent, f as ref, p as dayjs, a3 as onMounted, aC as IonGrid, af as IonRow, aB as IonCol, Q as normalizeClass, e0 as checkboxOutline, H as createCommentVNode, cb as clipboardOutline, e1 as statsChartOutline, K as modalController, aN as useRouter, v as documentTextOutline, c as computed, bO as useRoute, aI as IonContent, e2 as ellipsisVerticalSharp, d4 as person, aK as IonAccordionGroup, aJ as IonAccordion, P as normalizeStyle, G as closeCircleOutline, di as IonSegment, dj as IonSegmentButton, al as IonPopover, b1 as personOutline, b3 as printOutline, d5 as add, bx as IonPage, cT as defineAsyncComponent, cJ as __vitePreload } from './vendor-8vOj_QAN.js';
import { s as storeToRefs } from './pinia-Drf8jDfn.js';
import { al as ProgramService, _ as _export_sfc, P as PatientService, am as PatientProgramService, ad as GlobalPropertyService, r as StandardModal, o as createModal, an as _sfc_main$5, ao as ArtTasks, u as useDemographicsStore, F as DynamicButton, K as ObservationService, b as EncounterTypeId, G as toastSuccess, H as HisDate, ap as router, a as useProgramStore, h as useWindowSize, T as Toolbar, M as ProgramId, S as Service, aq as iconBMI, k as alertConfirmation, ar as openVisit, q as usePatientList, v as closeVisit, t as toastWarning, as as SetProgramService, N as ConceptService, at as getActiveVisit } from '../index-Bn2sysQf.js';
import { u as useUserActivities } from './useUserActivities-7xoS4Ne9.js';
import { u as usePatientProfile } from './usePatientProfile-4xgJeVsI.js';
import { D as DemographicBar } from './DemographicBar-CJWnjE0e.js';
import { i as isTaskPermitted } from './tasks-DVEYiPUi.js';
import { E as EnrollmentService, A as ANCRedirection } from './anc_service-BEQve71k.js';
...
const ArtCard = /* @__PURE__ */ _export_sfc(_sfc_main$4, [["__scopeId", "data-v-7ec7b45c"]]);

const _hoisted_1$3 = { class: "tasks-container" };
...
const TaskView = /* @__PURE__ */ _export_sfc(_sfc_main$3, [["__scopeId", "data-v-0fbb33f7"]]);
...
const _sfc_main$2 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const patientFullName = computed(() => {
      const pi = patient.value?.personInformation;
      if (!pi) return "this patient";
      const fromFull = typeof pi.fullName === "string" ? pi.fullName.trim() : "";
      if (fromFull) return fromFull;
      const parts = [pi.given_name, pi.middle_name, pi.family_name].filter((p) => p && String(p).trim());
      const joined = parts.map((p) => String(p).trim()).join(" ");
      return joined || "this patient";
    });
    const router = useRouter();
    const OnSubmit = async () => {
      const data = {
        "Patient enrolled": "Yes",
        obsValueType: {
          "Patient enrolled": "value_coded"
        }
      };
      if (await ObservationService.buildSaveObs(data, EncounterTypeId.LABOUR_ENROLLMENT)) {
        await EnrollmentService.createEnrollment();
        toastSuccess("Enrollment in Labour and Delivery program successful");
        modalController.dismiss();
    ...
    return (_ctx, _cache) => {
      return openBlock(), createBlock(StandardModal, {
        ...
        subtitle: "Labour and Delivery program enrollment  "
      }, {
        "top-buttons": withCtx(() => [
          createVNode(DynamicButton, {
            name: "Yes",
            "onClicked:btn": OnSubmit,
            fill: "solid",
            class: "submit-button"
          })
        ]),
        content: withCtx(() => [
          createBaseVNode("div", null, [
            createBaseVNode("p", _hoisted_1$2, 'Are you sure you want to enroll "' + toDisplayString(patientFullName.value) + '" in the Labour and Delivery program?', 1)
          ])
        ]),
        _: 1
      }, 8, ["headerIcon"]);
...
const LabourEnrollmentModal = /* @__PURE__ */ _export_sfc(_sfc_main$2, [["__scopeId", "data-v-8875df98"]]);

const LabourRedirection = async () => {
  const enrollmentData = await ObservationService.getObsByEncounterId(EncounterTypeId.LABOUR_ENROLLMENT);
  const latestEnrollmentObs = await ObservationService.getLatestObsByEncounterId(EncounterTypeId.LABOUR_ENROLLMENT);
  const latestEndLabourObs = await ObservationService.getLatestObsByEncounterId(EncounterTypeId.END_LABOUR_PROGRAM);
  const obstetricHistory = await ObservationService.getObsByEncounterId(EncounterTypeId.OBSTETRIC_HISTORY);
  const labourAssessment = await ObservationService.getObsByEncounterId(EncounterTypeId.LABOUR_ASSESSMENT);
  const labourVisit = await ObservationService.getObsByEncounterId(EncounterTypeId.Labour_and_delivery_visit);
  const latestEnrollmentDate = latestEnrollmentObs?.[0]?.obs_datetime ? new Date(latestEnrollmentObs[0].obs_datetime) : null;
  const latestEndLabourDate = latestEndLabourObs?.[0]?.obs_datetime ? new Date(latestEndLabourObs[0].obs_datetime) : null;
  const visitData = [...obstetricHistory, ...labourAssessment, ...labourVisit];
  ...
  const shouldShowEnrollmentModal = !hasEnrollmentData || latestEndLabourDate && latestEnrollmentDate && latestEndLabourDate > latestEnrollmentDate;
  if (shouldShowEnrollmentModal) {
    const birthdate = useDemographicsStore().getPatient()?.personInformation?.birthdate;
    const showUnderageWarning = birthdate ? HisDate.getAgeInYears(birthdate) < 9 : false;
    await createModal(LabourEnrollmentModal, { class: "small-confirm-modal " }, true, { showUnderageWarning });
...
const _hoisted_1$1 = { class: "enrollment-confirm-body" };
const _hoisted_2$1 = { class: "modal-content" };
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  __name: "PNCEnrollmentModal",
  props: {
    isOpen: { type: Boolean },
    closeModalFunc: {},
    onYes: {},
  ...
  setup(__props) {
    ...
    const OnSubmit = async () => {
      ...
      if (await ObservationService.buildSaveObs(data, EncounterTypeId.PNC_ENROLLMENT)) {
        await EnrollmentService.createEnrollment();
        toastSuccess("Enrollment in Postnatal Care program successful");
        modalController.dismiss();
    ...
    return (_ctx, _cache) => {
      return openBlock(), createBlock(StandardModal, {
        title: "Confirmation",
        headerIcon: unref(documentTextOutline),
        subtitle: "Postnatal Care program enrollment"
      }, {
        "top-buttons": withCtx(() => [
          createVNode(DynamicButton, {
            name: "Yes",
        ...
        content: withCtx(() => [
          createBaseVNode("div", _hoisted_1$1, [
            createBaseVNode("p", _hoisted_2$1, 'Are you sure you want to enroll "' + toDisplayString(patientFullName.value) + '" in the Postnatal Care (PNC) program?', 1)
          ])
        ]),
        _: 1
      }, 8, ["headerIcon"]);
...
const PNCEnrollmentModal = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["__scopeId", "data-v-300457fb"]]);

const PNCRedirection = async () => {
  const enrollmentData = await ObservationService.getObsByEncounterId(EncounterTypeId.PNC_ENROLLMENT);
  const latestEnrollmentObs = await ObservationService.getLatestObsByEncounterId(EncounterTypeId.PNC_ENROLLMENT);
  const latestEndPncObs = await ObservationService.getLatestObsByEncounterId(EncounterTypeId.END_PNC_PROGRAM);
  const postnatalDetails = await ObservationService.getObsByEncounterId(EncounterTypeId.POSTNATAL_DETAILS);
  const pncVisit = await ObservationService.getObsByEncounterId(EncounterTypeId.PNC_VISIT);
  const postnatalWardMother = await ObservationService.getObsByEncounterId(EncounterTypeId.POSTNATAL_WARD_STAY_MOTHER);
  const latestEnrollmentDate = latestEnrollmentObs?.[0]?.obs_datetime ? new Date(latestEnrollmentObs[0].obs_datetime) : null;
  const latestEndPncDate = latestEndPncObs?.[0]?.obs_datetime ? new Date(latestEndPncObs[0].obs_datetime) : null;
  const visitData = [...postnatalDetails, ...pncVisit, ...postnatalWardMother];
  const hasEnrollmentData = enrollmentData.some((item) => item.obs && item.obs.length > 0);
  const hasVisitData = visitData.some((item) => item.obs && item.obs.length > 0);
  const shouldShowEnrollmentModal = !hasEnrollmentData || latestEndPncDate && latestEnrollmentDate && latestEndPncDate > latestEnrollmentDate;
  if (shouldShowEnrollmentModal) {
    await createModal(PNCEnrollmentModal, { class: "small-confirm-modal " }, true);
  } else if (!hasVisitData) {
    router.push("/pnc/postnatal-details");
  } else {
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    const LabOrdersList = defineAsyncComponent(() => __vitePreload(() => import('./LabOrdersList-nWlc0PR0.js'),true              ?[]:void 0));
    const DiagnosesHistory = defineAsyncComponent(() => __vitePreload(() => import('./DiagnosesHistory-DWrrHX_C.js'),true              ?[]:void 0));
    const AllergiesHistory = defineAsyncComponent(() => __vitePreload(() => import('./AllergiesHistory-BDUB4OzA.js'),true              ?[]:void 0));
    const MedicationsHistory = defineAsyncComponent(() => __vitePreload(() => import('./MedicationsHistory-BNe-NMUT.js'),true              ?[]:void 0));
    const VisitsHistory = defineAsyncComponent(() => __vitePreload(() => import('./VisitsHistory-BReErjQv.js'),true              ?[]:void 0));
    const VitalsMeasurementsSummary = defineAsyncComponent(() => __vitePreload(() => import('./VitalsMeasurementsSummary-BuMNPOde.js'),true              ?[]:void 0));
    const PatientProfile = defineAsyncComponent(() => __vitePreload(() => import('./PatientProfile-BbI5TptF.js'),true              ?[]:void 0));
    const HTSPatientProfile = defineAsyncComponent(() => __vitePreload(() => import('./HTSPatientProfile-BLQl6YWp.js'),true              ?[]:void 0));
    const NeonatalPatientProfile = defineAsyncComponent(() => __vitePreload(() => import('./NeonatalPatientProfile-DeWOMlnk.js'),true              ?[]:void 0));
    const AETCPatientProfile = defineAsyncComponent(() => __vitePreload(() => import('./AETCPatientProfile-CUHqtRk6.js'),true              ?[]:void 0));
    const WeightHeightChart = defineAsyncComponent(() => __vitePreload(() => import('./WeightHeightChart-msINhiFg.js'),true              ?[]:void 0));
    const PreviousVitals = defineAsyncComponent(() => __vitePreload(() => import('../index-Bn2sysQf.js').then(n => n.cV),true              ?[]:void 0));
    const BloodPressure = defineAsyncComponent(() => __vitePreload(() => import('./BloodPressure-BdIoqxIr.js'),true              ?[]:void 0));
    const router = useRouter();
      ...
      try {
        if (await alertConfirmation(`Are you sure you want to create the visit?`)) {
          ...
          if (await openVisit(demographicsStore.patient)) {
            ...
            toastSuccess("The patient visit is now active. Patient is on the waiting list for vitals", 5e3, "top-left");
          }
        }
      } catch (error) {
      ...
      try {
        if (await alertConfirmation(`Are you sure you want to close the visit?`)) {
          ...
          if (await closeVisit(patient.value)) {
            ...
            await usePatientList().refresh(Service.getUserLocationId());
            toastSuccess("The patient visit is now closed", 5e3, "top-left");
          }
        }
      } catch (error) {
    ...
    const weightToKg = (valueNumeric) => {
      if (valueNumeric == null || valueNumeric === void 0) return valueNumeric;
      return valueNumeric > 10 ? valueNumeric / 1e3 : valueNumeric;
    };
    const updateVitalsData = async () => {
      ...
      const weightGrams = await ConceptService.getConceptID("Weight (grams)");
      ...
      const vitals = await ObservationService.getLatestObsByEncounterIdAndConcepts(6, [
        height,
        weightKg,
        weightGrams,
        systolic,
      ...
      vitals.forEach((item) => {
        ...
        } else if (item.concept_id === weightKg) {
          vitalsData.value.weight = item.value_numeric;
        } else if (item.concept_id === weightGrams && vitalsData.value.weightGrams == null) {
          vitalsData.value.weight = weightToKg(item.value_numeric);
                  ...
                  default: withCtx(() => [
                    ...
                    createVNode(unref(IonItem), {
                      ...
                      default: withCtx(() => [
                        ...
                        _cache[51] || (_cache[51] = createBaseVNode("span", { class: "sub-menu-txt" }, "Manage Relationship ", -1))
                      ]),
                      _: 1
                    }),
                    createVNode(unref(IonItem), {
...
const PatientProfile = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-c471efe5"]]);
