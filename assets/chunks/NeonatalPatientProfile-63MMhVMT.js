import { s as storeToRefs, d as defineStore } from './pinia-Byno_K7k.js';
import { s as defineComponent, aM as useRouter, a3 as onMounted, w as watch, y as openBlock, z as createElementBlock, C as createBaseVNode, A as createVNode, F as unref, aC as IonGrid, B as withCtx, af as IonRow, aB as IonCol, bM as IonCard, O as createBlock, bK as IonBadge, a5 as createTextVNode, D as toDisplayString, H as createCommentVNode, be as IonCardContent, dv as IonImg, bA as IonText, Q as normalizeClass, f as ref, c as computed, aH as IonContent, J as Fragment, S as renderList, d_ as IonSkeletonText, L as IonIcon, cH as arrowForward, N as IonButton, fm as reloadOutline, bT as withKeys, a8 as withModifiers, d4 as add, v as documentTextOutline, P as normalizeStyle, a9 as chevronDownOutline, b0 as personOutline, aW as femaleOutline, aT as medicalOutline, R as alertCircleOutline, aX as heartOutline, bt as pulseOutline, a$ as medkitOutline, eN as scanOutline, fn as restaurantOutline, fo as flaskOutline, et as thermometerOutline, fp as bandageOutline, aY as flagOutline, d3 as person, e1 as ellipsisVerticalSharp, fq as maleFemaleOutline, b9 as calendarOutline, b1 as locationOutline, fr as qrCodeOutline, ff as arrowForwardOutline, di as IonSegmentButton, e0 as statsChartOutline, a7 as IonLabel, dh as IonSegment, aJ as IonAccordionGroup, aI as IonAccordion, aq as IonItem, al as IonPopover } from './vendor-NYNX75Tj.js';
import { u as usePatientProfile } from './usePatientProfile-wupiV9d9.js';
import { u as useDemographicsStore, b as EncounterTypeId, S as Service, E as EncounterService, t as toastWarning, _ as _export_sfc, l as PreviousVitals, r as StandardModal, H as HisDate, o as createModal, bp as LabOrderService, K as ObservationService, M as ConceptService, P as PatientService, k as alertConfirmation } from '../index-DRy1cgfg.js';
...
import { I as IMAGES } from './images-_va7dFSn.js';
import WeightHeightChart from './WeightHeightChart-DrJX4fDN.js';
import { _ as _sfc_main$6 } from './EnterLabResultsModal.vue_vue_type_script_setup_true_lang-r6OleFyi.js';
import { N as NeonatalReportService } from './reportService-DpeQRggz.js';
...
const _sfc_main$5 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const getEncounterTypeNameById = (encounterTypeId) => {
      const encounterTypeMap = {
        [EncounterTypeId.NEONATAL_SIGNS_SYMPTOMS]: "NEONATAL SIGNS & SYMPTOMS",
        [EncounterTypeId.NEONATAL_REVIEW_OF_SYSTEMS]: "NEONATAL REVIEW OF SYSTEMS",
        [EncounterTypeId.PHYSICAL_EXAMINATION_BABY]: "PHYSICAL EXAMINATION BABY",
        [EncounterTypeId.VITALS]: "VITALS",
        [EncounterTypeId.NEONATAL_SYSTEMIC_EXAMINATION]: "NEONATAL SYSTEMIC EXAMINATION",
        [EncounterTypeId.NEONATAL_ENROLLMENT]: "NEONATAL ENROLLMENT",
        [EncounterTypeId.DIAGNOSIS]: "DIAGNOSIS",
        [EncounterTypeId.LAB_ORDERS]: "LAB ORDERS",
        [EncounterTypeId.TREATMENT]: "TREATMENT",
        [EncounterTypeId.NEONATAL_CLINICAL_REVIEW_OUTCOMES]: "NEONATAL CLINICAL REVIEW OUTCOMES",
        [EncounterTypeId.NEONATAL_TRIAGE]: "NEONATAL TRIAGE",
        [EncounterTypeId.NEONATAL_EMERGENCY_TRIAGE]: "NEONATAL EMERGENCY TRIAGE"
      };
      return encounterTypeMap[encounterTypeId] || null;
    };
    const getEncounterNamesFromDemographicsStore = () => {
      if (!patient.value) {
        return [];
      }
      const encounterTypeIds = /* @__PURE__ */ new Set();
      if (Array.isArray(patient.value.observations)) {
        patient.value.observations.forEach((obs) => {
          const encounterTypeId = obs?.encounter_type;
          if (typeof encounterTypeId === "number") {
            encounterTypeIds.add(encounterTypeId);
          }
        });
      }
      const encounterKeys = [
        "neonatal_signs_symptoms",
        "neonatal_review_of_systems",
        "neonatal_physical_examination_baby",
        "neonatal_vitals",
        "neonatal_systemic_examination",
        "neonatal_enrollments",
        "neonatal_diagnosis",
        "neonatal_lab_orders",
        "neonatal_treatment",
        "neonatal_clinical_review_outcomes",
        "neonatal_triage",
        "neonatal_emergency_triage"
      ];
      encounterKeys.forEach((key) => {
        const encounterData = patient.value[key];
        if (encounterData) {
          if (Array.isArray(encounterData.saved)) {
            encounterData.saved.forEach((enc) => {
              const encTypeId = enc?.encounter_type_id;
              if (typeof encTypeId === "number") {
                encounterTypeIds.add(encTypeId);
              }
            });
          }
          if (Array.isArray(encounterData.unsaved)) {
            encounterData.unsaved.forEach((enc) => {
              const encTypeId = enc?.encounter_type_id;
              if (typeof encTypeId === "number") {
                encounterTypeIds.add(encTypeId);
              }
      ...
      encounterTypeIds.forEach((id) => {
        const name = getEncounterTypeNameById(id);
        if (name) {
          encounterNames.push(name);
        }
      });
    ...
    ADMISSION_WORKFLOW_STEPS[0].route;
    ...
    const hasLabOrders = () => {
      if (!patient.value?.labOrders) return false;
      const { saved = [], unsaved = [] } = patient.value.labOrders;
      return saved.length > 0 || unsaved.length > 0;
    };
    ...
    const loadNextEncounterBadge = async () => {
      ...
      try {
        const encounterNamesFromStore = getEncounterNamesFromDemographicsStore();
        ...
        const combinedEncounters = [.../* @__PURE__ */ new Set([...encounterNamesFromStore, ...generalEncounterNames])];
        if (hasLabOrders()) {
          combinedEncounters.push("LAB ORDERS");
        }
        ...
        const hasTriage = hasTriageEncounter(combinedEncounters);
        if (!hasTriage) {
          nextEncounterBadge.value = "Triage";
        } else {
          const nextTitle = determineNextEncounterTitle(combinedEncounters);
    ...
    const handleAdmit = async () => {
      ...
      try {
        ...
        const completedCount = getSequentiallyCompletedStepCount(combinedEncounters, ADMISSION_WORKFLOW_STEPS);
        const nextStep = ADMISSION_WORKFLOW_STEPS[completedCount];
        if (nextStep) {
          await router.push(nextStep.route);
        } else {
          await router.push("/neonatal/clinical-review-outcomes");
        }
      } catch (error) {
...
const NeonatalCare = /* @__PURE__ */ _export_sfc(_sfc_main$5, [["__scopeId", "data-v-2ea5c938"]]);
...
const useNeonatalPatientProfileStore = defineStore("neonatalPatientProfile", () => {
  ...
  const workflowSteps = [
    ...
    },
    {
      ...
      title: "Clinical Review Outcomes",
      route: "/neonatal/clinical-review-outcomes",
      encounterNames: ["NEONATAL ADMISSION OUTCOMES", "ADMISSION OUTCOMES", "NEONATAL CLINICAL REVIEW OUTCOMES", "CLINICAL REVIEW OUTCOMES"],
      isCompleted: false
    },
    {
  ...
  const getEncounterNamesFromPatient = (patient) => {
    if (!patient) {
      return [];
    }
    const encounterTypeIds = /* @__PURE__ */ new Set();
    ...
    encounterKeys.forEach((key) => {
      const encounterData = patient[key];
      if (encounterData) {
        if (Array.isArray(encounterData.saved)) {
          encounterData.saved.forEach((enc) => {
            const encTypeId = enc?.encounter_type_id;
  ...
  const hasLabOrders = (patient) => {
    if (!patient?.labOrders) return false;
    const { saved = [], unsaved = [] } = patient.labOrders;
    return saved.length > 0 || unsaved.length > 0;
  };
  const determineNextWorkflowStep = (combinedNames) => {
    const normalizeEncounterName2 = (name) => (name ?? "").trim().toUpperCase();
    const normalizedEncounterSet = new Set(combinedNames.map((name) => normalizeEncounterName2(name)));
    const hasEncounter = (names) => names.some((name) => normalizedEncounterSet.has(normalizeEncounterName2(name)));
    const isTriageCompleted = hasEncounter([
      "NEONATAL TRIAGE",
      "NEONATAL EMERGENCY TRIAGE",
      "EMERGENCY TRIAGE",
      "TRIAGE",
    ...
    const completedCount = getSequentiallyCompletedStepCount(combinedNames, workflowSteps);
  ...
  const fetchExistingOutcome = async (patientId, patient) => {
    try {
      if (!patientId) return;
      const encounterNamesFromStore = getEncounterNamesFromPatient(patient);
      let generalEncounterNames = [];
      const isOffline = Service.getPouchDbStatus() || Service.getLanConnectionStatus();
      try {
      ...
      const combinedNames = [.../* @__PURE__ */ new Set([...encounterNamesFromStore, ...generalEncounterNames])];
      if (hasLabOrders(patient)) {
        combinedNames.push("LAB ORDERS");
      }
      nextWorkflowStep.value = determineNextWorkflowStep(combinedNames);
      const normalizedNames = combinedNames.map((n) => n.toUpperCase().trim());
      if (normalizedNames.includes("NEONATAL DISCHARGE") || normalizedNames.includes("DISCHARGE")) {
        customOutcomeLabel.value = "Discharged";
        selectedStatus.value = 6;
        return;
      }
      const hasOutcomeEncounter = normalizedNames.some(
      ...
      const workflowNames = normalizedNames.filter(
      ...
      if (nextWorkflowStep.value === "Clinical Review Sign Off") {
        customOutcomeLabel.value = "Clinical Review Sign Off";
        return;
      }
      ...
      nextWorkflowStep.value = "";
    }
  };
  const validateReportGeneration = async (patientId, type, patient) => {
    try {
      const encounterNamesFromStore = patient ? getEncounterNamesFromPatient(patient) : [];
      let generalEncounterNames = [];
      const isOffline = Service.getPouchDbStatus() || Service.getLanConnectionStatus();
      try {
        const allEncounters = await EncounterService.getEncounters(patientId);
      ...
      const combined = [.../* @__PURE__ */ new Set([...encounterNamesFromStore, ...generalEncounterNames])];
      if (patient && hasLabOrders(patient)) {
        combined.push("LAB ORDERS");
      }
      if (type === "clinical") {
        const steps = [
          { encounterNames: ["NEONATAL SIGNS & SYMPTOMS", "NEONATAL SIGNS SYMPTOMS"] },
  ...
  const loadPatientData = async (patient) => {
    ...
    checkAge(patient);
    await Promise.all([getLabResults(patient), getReferralTests(patient.patientID), fetchExistingOutcome(patient.patientID, patient), getReferrals()]);
  };
...
const _hoisted_21 = { class: "field-value next-step" };
const _hoisted_22 = { class: "graphSection" };
const _hoisted_23 = { class: "tabs-control" };
const _hoisted_24 = { class: "tab-button-content" };
const _hoisted_25 = { class: "tab-button-content" };
const _hoisted_26 = { class: "tab-button-content" };
const _hoisted_27 = { class: "tabs-content" };
const _hoisted_28 = { slot: "content" };
const _sfc_main = /* @__PURE__ */ defineComponent({
  __name: "NeonatalPatientProfile",
  setup(__props) {
    const router = useRouter();
    const profileStore = useNeonatalPatientProfileStore();
    const { checkUnderSixWeeks, outcomeStatusLabel, outcomeStatusClass, nextWorkflowStep } = storeToRefs(profileStore);
    ...
    const generateReport = async (type) => {
      handleMenuAction(async () => {
        ...
        try {
          const validationResult = await profileStore.validateReportGeneration(patient.value.patientID, type, patient.value);
            ...
            createBaseVNode("div", _hoisted_4, [
              ...
              createBaseVNode("div", _hoisted_6, [
                ...
                createBaseVNode("div", _hoisted_17, [
                  ...
                  createBaseVNode("span", _hoisted_18, toDisplayString(unref(patient).ID || unref(patient).id), 1)
                ]),
                ...
                unref(nextWorkflowStep) ? (openBlock(), createElementBlock("div", _hoisted_20, [
                  createVNode(unref(IonIcon), {
                    icon: unref(arrowForwardOutline),
                    class: "detail-icon"
                  }, null, 8, ["icon"]),
                  _cache[13] || (_cache[13] = createBaseVNode("span", { class: "field-label" }, "Next Step:", -1)),
                  createBaseVNode("span", _hoisted_21, toDisplayString(unref(nextWorkflowStep)), 1)
                ])) : createCommentVNode("", true)
              ])
            ])
          ]),
...
const NeonatalPatientProfile = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-bc2033cf"]]);
