import { s as defineComponent, f as ref, w as watch, y as openBlock, z as createElementBlock, J as Fragment, S as renderList, A as createVNode, F as unref, af as IonRow, B as withCtx, aB as IonCol, Q as normalizeClass, a7 as IonLabel, a5 as createTextVNode, D as toDisplayString, C as createBaseVNode, O as createBlock, eg as pencilOutline, eD as trashBinOutline, H as createCommentVNode, a3 as onMounted, bM as IonCard, bc as IonCardHeader, L as IonIcon, R as alertCircleOutline, be as IonCardContent, a6 as IonInput, N as IonButton, G as closeCircleOutline, c as computed, aq as IonItem, ae as IonCheckbox, a8 as withModifiers, bK as IonBadge, ak as IonDatetime, aE as IonToolbar, aF as IonTitle, aG as IonMenu, ap as IonList, I as IonHeader, aH as IonContent, b9 as calendarOutline, x as resolveComponent } from './vendor-CZkQjgFf.js';
import { F as DynamicButton, _ as _export_sfc, bo as useAllegyStore, u as useDemographicsStore, a7 as useUserStore, M as ConceptService, bE as searchHealthcareEquipmentAllergies, bF as concatenateArrays, aC as ListPicker, S as Service, K as ObservationService, b as EncounterTypeId, G as toastSuccess, t as toastWarning, B as BasicInputField, bG as useNextAppointmentStore, H as HisDate, A as AppointmentService, bH as DrugOrderService, o as createModal, b4 as confirmModal, f as useStatusStore, aK as useClinicalDaysStore, V as useGlobalPropertyStore, aL as setValueProps, P as PatientService, a as useProgramStore, n as icons, b2 as useTreatmentPlanStore, be as useNCDMedicationsStore } from '../index-HUvHuTRi.js';
import { a as useConfirm } from './usePatientProfile-dy_Xb7Qj.js';
import { l as lodashExports } from './lodash-BwW3y_1X.js';
import { u as useNonPharmaTherapyStore, g as getOPDMedicationRunOutDate } from './nonPharmaTherapyStore-B_bIrS5M.js';
import { m as mapState } from './pinia-CBCkT7wa.js';
...
const _sfc_main$3 = /* @__PURE__ */ defineComponent({
  ...
  emits: ["remove-item", "edit-item"],
  setup(__props, { emit: __emit }) {
    const props = __props;
    const { showConfirm } = useConfirm();
    const emit = __emit;
    const localMedicalDrugsList = ref([...props._selectedMedicalDrugsList]);
    watch(() => props._selectedMedicalDrugsList, (newVal) => {
      localMedicalDrugsList.value = [...newVal];
    }, { deep: true });
    const highlightItem = (index) => {
      const el = document.getElementById(index + "_lbl");
      if (el) {
        el.style.color = "#006401";
      }
      highlightActionBtns(index);
    };
    const undoHighlightItem = (index) => {
      const el = document.getElementById(index + "_lbl");
      if (el) {
        el.style.color = "#636363";
      }
      undohighlightActionBtns(index);
    };
    const highlightActionBtns = (index) => {
      const elements = document.getElementsByClassName(index + "_spanlbl");
      for (let i = 0; i < elements.length; i++) {
        elements[i].style.display = "inline-block";
      }
    };
    const undohighlightActionBtns = (index) => {
      const elements = document.getElementsByClassName(index + "_spanlbl");
      for (let i = 0; i < elements.length; i++) {
        elements[i].style.display = "none";
      }
    };
    const asignLblID = (num) => {
      return num + "_lbl";
    };
    const asignSpanLblID = (num) => {
      return num + "_spanlbl";
    };
    const removeItemAtIndex = async (index) => {
      const deleteConfirmed = await showConfirm("Are you sure you want to remove the prescription?");
      if (deleteConfirmed) {
        emit("remove-item", index);
      }
    };
    const editItemAtIndex = (index) => {
      emit("edit-item", index);
    };
    const highLightBnd = (item) => {
      if (item && item.highlightbackground === true) {
        return props.highLightBackground;
      }
      return "";
    };
    return (_ctx, _cache) => {
      return openBlock(true), createElementBlock(Fragment, null, renderList(localMedicalDrugsList.value, (item, index) => {
        return openBlock(), createElementBlock("div", {
          id: "df",
          class: "medication-item",
          key: index,
          onMousemove: ($event) => highlightItem(index),
          onMouseout: ($event) => undoHighlightItem(index)
        }, [
          createVNode(unref(IonRow), { class: "medication-header" }, {
            default: withCtx(() => [
              createVNode(unref(IonCol), {
                class: normalizeClass(["ion-text-wrap", highLightBnd(item)])
              }, {
                default: withCtx(() => [
          ...
          createVNode(unref(IonRow), { class: "medication-details" }, {
            default: withCtx(() => [
              createVNode(unref(IonCol), {
                size: "10",
                class: normalizeClass(["ion-text-wrap details-col", highLightBnd(item)])
              }, {
                default: withCtx(() => [
                  createVNode(unref(IonLabel), { class: "details-text" }, {
                    default: withCtx(() => [
              ...
              __props.show_actions_buttons ? (openBlock(), createBlock(unref(IonCol), {
                key: 0,
                class: normalizeClass([highLightBnd(item), "actions-col"]),
                size: "2"
              }, {
                default: withCtx(() => [
                  createVNode(DynamicButton, {
                    ...
                    onClick: ($event) => editItemAtIndex(index),
                    class: normalizeClass(["action-button", asignSpanLblID(index)])
...
const DynamicList = /* @__PURE__ */ _export_sfc(_sfc_main$3, [["__scopeId", "data-v-ac1161a7"]]);
...
const NonPharmacologicalIntervention = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["__scopeId", "data-v-6e6627b1"]]);
