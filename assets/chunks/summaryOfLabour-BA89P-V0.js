import { c as computed, s as defineComponent, y as openBlock, z as createElementBlock, A as createVNode, B as withCtx, F as unref, be as IonCardContent, bM as IonCard, aM as useRouter, O as createBlock, aH as IonContent, C as createBaseVNode, b_ as chevronBackOutline, bw as IonPage, f as ref } from './vendor-BcieWP-_.js';
import { n as icons, y as StandardValidations, z as StandardForm, C as useExposeFromStandardForm, t as toastWarning, K as ObservationService, b as EncounterTypeId, G as toastSuccess, _ as _export_sfc, T as Toolbar, F as DynamicButton } from '../index-D-gMOJ4F.js';
import { D as DemographicBar } from './DemographicBar-CCI_pinu.js';
import { B as BasicFooter } from './BasicFooter-DYZy49LD.js';
...
const computeTotalDuration = (formValues) => {
  const totalMinutes = toNum$1(formValues[SUMMARY_LABOUR_STAGE_KEYS.first.hours]) * 60 + toNum$1(formValues[SUMMARY_LABOUR_STAGE_KEYS.first.minutes]) + toNum$1(formValues[SUMMARY_LABOUR_STAGE_KEYS.second.hours]) * 60 + toNum$1(formValues[SUMMARY_LABOUR_STAGE_KEYS.second.minutes]) + toNum$1(formValues[SUMMARY_LABOUR_STAGE_KEYS.third.hours]) * 60 + toNum$1(formValues[SUMMARY_LABOUR_STAGE_KEYS.third.minutes]);
...
const SECOND_STAGE_MDT_ALERT_THRESHOLD_MINUTES = 60;
  ...
  const summaryOfLabourFormSection = computed(() => {
    return [
      ...
      },
      {
        ...
        placeholder: "Minutes (0–59)",
        grid: { s: "12", md: "6" },
        validation: (value) => validateMinutesWithMax(value, "2nd stage (minutes)", 60),
        onChange: updateTotalDurationOnChange
      },
      {
        componentType: "Alert",
        value: "Involve disciplinary team",
        backgroundColor: "#FECDCA",
        textColor: "#B42318",
        condition: (vals) => {
          return toNum(vals?.["2nd stage duration (minutes)"]) > SECOND_STAGE_MDT_ALERT_THRESHOLD_MINUTES;
        },
        grid: { s: "12" }
      },
      {
