import { S as Service, H as HisDate, ai as ProgramService, M as ProgramId, aY as AppEncounterService, b as EncounterTypeId, K as ObservationService } from '../index-xpeKIrss.js';
...
class NeonatalService extends Service {
  ...
  static async dischargeNeonate(patientId, dischargeData, date = HisDate.sessionDate()) {
    try {
      ...
      const providerId = Service.getUserID() || -1;
      const encounter = new AppEncounterService(patientId, EncounterTypeId.DISCHARGE_PATIENT, providerId);
      encounter.setDate(date);
      await encounter.createEncounter();
      if (observations.length > 0) {
  ...
  static async buildDischargeObservations(data, date) {
    ...
    if (data.medications && data.medications.length > 0) {
      ...
      for (const med of data.medications) {
        medicationSummary += `${med.name} (${med.dosage || "-"} ${med.frequency || "-"}); `;
      }
      await pushText("Medication orders", medicationSummary);
    }
    if (data.clinicFollowUp) {
      await pushText("Next facility to visit", data.clinicFollowUp);
    }
    await pushText("Health education", data.healthEducationGiven);
