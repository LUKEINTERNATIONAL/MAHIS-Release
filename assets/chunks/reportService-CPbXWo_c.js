import { t as triageConceptMapping } from './triage-qcMfqQ0N.js';
import { b as EncounterTypeId, S as Service, u as useDemographicsStore, a7 as useUserStore, a_ as getOfflineSavedUnsavedData, M as ConceptService, E as EncounterService, O as OrderService, ci as DatabaseManager, g as getPouchDBRecords, bH as DrugOrderService, P as PatientService, H as HisDate } from '../index-DK2HCmpA.js';
import { N as NeonatalService } from './neonatal_service-B2tqUHLo.js';
import { bG as QRCode, cA as E, bD as html2canvas } from './vendor-DEu2hKw1.js';
...
class NeonatalReportService {
  ...
  /**
   * Clear all caches for a specific patient. Call this after creating new encounters
   * to ensure fresh data is fetched when generating reports.
   */
  static clearCacheForPatient(patientId) {
    const patientIdStr = String(patientId);
    for (const key of this.encountersCache.keys()) {
      if (key.startsWith(`${patientIdStr}|`)) {
        this.encountersCache.delete(key);
      }
    }
    this.ordersCache.delete(patientIdStr);
    this.drugOrdersCache.delete(patientIdStr);
    this.enrollmentCache.delete(patientIdStr);
    this.patientCache.delete(patientIdStr);
  }
  ...
  static buildPatientInfo(patient) {
    ...
    const npidObj = identifiers.find(
      (id) => id.identifierType?.display === "National ID" || id.identifier_type === 3 || id.type?.name === "National ID"
    );
    ...
    if (birthdate) {
      ...
      const ageDate = new Date(Date.now() - birthDateObj.getTime());
      const calculatedAge = Math.abs(ageDate.getUTCFullYear() - 1970);
      if (calculatedAge === 0) {
        const diffTime = Math.abs(Date.now() - birthDateObj.getTime());
    ...
    let homeAddress = "N/A";
    const addresses = patient?.person?.addresses || [];
    if (addresses && addresses.length > 0) {
      const curr = addresses[0];
      const currParts = [curr.city_village, curr.township_division, curr.state_province].filter(Boolean);
      if (currParts.length > 0) currentAddress = currParts.join(", ");
      if (addresses.length > 1) {
        const home = addresses[1];
        const homeParts = [home.city_village, home.township_division, home.state_province].filter(Boolean);
        if (homeParts.length > 0) homeAddress = homeParts.join(", ");
      }
    } else if (patient?.personInformation?.current_district) {
      const p = patient.personInformation;
      const currParts = [p.current_village, p.current_traditional_authority, p.current_district].filter(Boolean);
      if (currParts.length > 0) currentAddress = currParts.join(", ");
      const homeParts = [p.home_village, p.home_traditional_authority, p.home_district].filter(Boolean);
      if (homeParts.length > 0) homeAddress = homeParts.join(", ");
    }
    let occupation = "N/A";
    let education = "N/A";
    let religion = "N/A";
    let maritalStatus = "N/A";
    const attributes = patient?.person?.attributes || patient?.attributes || [];
    for (const attr of attributes) {
      const attrType = attr?.attributeType?.display || attr?.type?.name || attr?.attribute_type?.name;
      const val = attr?.value || "";
      if (!attrType || !val) continue;
      if (attrType.toLowerCase().includes("occupation")) {
        occupation = val?.display || val;
      } else if (attrType.toLowerCase().includes("education")) {
        education = val?.display || val;
      } else if (attrType.toLowerCase().includes("religion")) {
        religion = val?.display || val;
      } else if (attrType.toLowerCase().includes("marital")) {
        maritalStatus = val?.display || val;
      }
    }
    return {
      fullName,
  ...
  static isOffline() {
    return Service.getPouchDbStatus() || Service.getLanConnectionStatus();
  }
  static async getOfflinePatientRecord(patientId) {
    try {
      const demographicsStore = useDemographicsStore();
      let patient = demographicsStore.getPatient();
      const currentId = patient?.patient_id ?? patient?.patientID ?? patient?.ID;
      if (!patient || !currentId || String(currentId) !== String(patientId)) {
        patient = await demographicsStore.getPatientData(patientId);
      }
      return patient || null;
    } catch (error) {
      console.warn("[ReportService] Failed to resolve offline patient record", error);
      return null;
    }
  }
  ...
  static matchesPatientId(record, patientId) {
    if (!record) return true;
    const raw = record?.patient_id ?? record?.patientID ?? record?.ID;
    if (raw === void 0 || raw === null) return true;
    return String(raw) === String(patientId);
  }
  static extractObsListFromEntry(entry) {
    if (!entry || typeof entry !== "object") return [];
    const obsCandidates = entry?.observations ?? entry?.obs ?? entry?.observations_list ?? entry?.observation ?? entry?.data?.observations;
    if (Array.isArray(obsCandidates)) return obsCandidates;
    if (obsCandidates && typeof obsCandidates === "object") return [obsCandidates];
    const hasObsLikeValue = entry?.concept_id !== void 0 || entry?.concept_name !== void 0 || entry?.value_text !== void 0 || entry?.value_numeric !== void 0 || entry?.value_coded !== void 0 || entry?.value_datetime !== void 0;
    return hasObsLikeValue ? [entry] : [];
  }
  static buildOfflineEncountersFromPatientRecord(patientId) {
    const sections = [
      { key: "neonatal_enrollments", typeId: EncounterTypeId.NEONATAL_ENROLLMENT, name: "NEONATAL ENROLLMENT" },
      { key: "neonatal_emergency_triage", typeId: EncounterTypeId.NEONATAL_EMERGENCY_TRIAGE, name: "NEONATAL EMERGENCY TRIAGE" },
      { key: "neonatal_triage", typeId: EncounterTypeId.NEONATAL_TRIAGE, name: "NEONATAL TRIAGE" },
      { key: "neonatal_signs_symptoms", typeId: EncounterTypeId.NEONATAL_SIGNS_SYMPTOMS, name: "NEONATAL SIGNS & SYMPTOMS" },
      { key: "neonatal_review_of_systems", typeId: EncounterTypeId.NEONATAL_REVIEW_OF_SYSTEMS, name: "NEONATAL REVIEW OF SYSTEMS" },
      { key: "neonatal_physical_examination", typeId: EncounterTypeId.PHYSICAL_EXAMINATION_BABY, name: "PHYSICAL EXAMINATION BABY" },
      { key: "neonatal_physical_examination_baby", typeId: EncounterTypeId.PHYSICAL_EXAMINATION_BABY, name: "PHYSICAL EXAMINATION BABY" },
      { key: "neonatal_vitals", typeId: EncounterTypeId.VITALS, name: "VITALS" },
      { key: "neonatal_systemic_examination", typeId: EncounterTypeId.NEONATAL_SYSTEMIC_EXAMINATION, name: "NEONATAL SYSTEMIC EXAMINATION" },
      { key: "diagnosis", typeId: EncounterTypeId.DIAGNOSIS, name: "DIAGNOSIS" },
      { key: "treatment", typeId: EncounterTypeId.TREATMENT, name: "TREATMENT" },
      { key: "neonatal_clinical_review_outcomes", typeId: EncounterTypeId.NEONATAL_CLINICAL_REVIEW_OUTCOMES, name: "NEONATAL CLINICAL REVIEW OUTCOMES" },
      { key: "neonatal_discharges", typeId: EncounterTypeId.NEONATAL_DISCHARGE, name: "NEONATAL DISCHARGE" }
    ];
    const encounters = [];
    for (const section of sections) {
      const raw = getOfflineSavedUnsavedData(section.key);
      const list = Array.isArray(raw) ? raw : [];
      const filtered = list.filter((item) => this.matchesPatientId(item, patientId));
      const obs = [];
      for (const entry of filtered) {
        obs.push(...this.extractObsListFromEntry(entry));
      }
      if (!obs.length) continue;
      encounters.push({
        encounter_id: `offline_${section.key}_${patientId}`,
        encounter_type_id: section.typeId,
        encounter_type_name: section.name,
  ...
  static buildObsIndex(encounters) {
    ...
    let obsWithoutConcept = 0;
    for (const obs of allObs) {
      const names = this.getConceptNamesFromObs(obs);
      if (names.length === 0) {
        obsWithoutConcept++;
        if (obsWithoutConcept <= 3) {
          console.warn("  ⚠️ Observation without concept name:", {
            obs_id: obs?.obs_id || obs?.obsId || obs?.id,
            value: this.extractObsValue(obs),
            keys: Object.keys(obs || {})
          });
        }
      }
      for (const name of names) push(name, obs);
    }
    if (obsWithoutConcept > 0) {
      console.warn(`[ReportService] ⚠️ ${obsWithoutConcept} observations have no concept name!`);
    }
  ...
  static clearCache(patientId) {
    ...
    this.ordersCache.delete(pId);
    this.drugOrdersCache.delete(pId);
    this.enrollmentCache.delete(pId);
    this.patientCache.delete(pId);
  }
  ...
  static async getEncounters(patientId, params = {}) {
    ...
    const promise = (async () => {
      if (this.isOffline()) {
        const patient = await this.getOfflinePatientRecord(patientId);
        const observationsArray = Array.isArray(patient?.observations) ? patient.observations : [];
        const normalizedObservations = observationsArray.map((enc, idx) => {
          const obsList = enc.observations || enc.obs || [];
          const encounterTypeId = enc.encounter_type_id ?? enc.encounter_type ?? enc.type?.id;
          const encounterId = enc.encounter_id || enc.id || `obs_${encounterTypeId}_${idx}`;
          return {
            ...enc,
            ...
            encounter_datetime: enc.encounter_datetime || enc.encounterDatetime || enc.date_created || (/* @__PURE__ */ new Date()).toISOString()
          };
        });
        const legacyEncounters = this.buildOfflineEncountersFromPatientRecord(patientId);
        const allEncounters = [...normalizedObservations, ...legacyEncounters];
        return allEncounters;
      }
      return EncounterService.getEncounters(Number(patientId), params);
    })().catch((e) => {
      for (const k of this.encountersCache.keys()) {
        if (k.startsWith(`${patientId}|`)) this.encountersCache.delete(k);
      }
      throw e;
    });
  ...
  static async getLabOrders(patientId) {
    ...
    const promise = (async () => {
      if (this.isOffline()) {
        const patient = await this.getOfflinePatientRecord(patientId);
        const labOrders = [
          ...patient?.labOrders?.saved || [],
          ...patient?.labOrders?.unsaved || []
        ];
        if (labOrders.length) return labOrders;
      }
      return OrderService.getOrders(Number(patientId));
    })().catch((e) => {
  ...
  static async getDrugOrders(patientId, encountersRaw) {
    const key = String(patientId);
    ...
    const promise = (async () => {
      if (this.isOffline()) {
        ...
        if (Array.isArray(encountersRaw)) {
          for (const encounter of encountersRaw) {
            if (encounter?.drug_orders && Array.isArray(encounter.drug_orders)) {
              allDrugOrders.push(...encounter.drug_orders);
            }
          }
        }
        if (allDrugOrders.length > 0) {
          return allDrugOrders;
        }
        if (!DatabaseManager.databaseNames.includes("drug_orders")) {
          return [];
        }
        let records = [];
        try {
          const all = await getPouchDBRecords("drug_orders", { selector: {} });
          records = Array.isArray(all) ? all : all?.records || [];
        } catch (error) {
          console.warn("[ReportService] Unable to read offline drug orders", error);
          return [];
        }
        const encounterIds = /* @__PURE__ */ new Set();
        if (Array.isArray(encountersRaw)) {
          for (const encounter of encountersRaw) {
            const id = encounter?.encounter_id ?? encounter?.id;
            if (id !== void 0 && id !== null) encounterIds.add(String(id));
          }
        }
        const patientPrefix = `${patientId}_`;
        const filtered = (records || []).filter((record) => {
          const encId = record?.encounter_id;
          if (encounterIds.size > 0) return encounterIds.has(String(encId));
          if (typeof encId === "string") return encId.startsWith(patientPrefix);
          return false;
        });
        return filtered.flatMap((record) => record?.drug_orders || []);
      }
      return DrugOrderService.getAllDrugOrders(Number(patientId), 500);
    })().catch((e) => {
  ...
  static async getClinicalReviewReportData(patientId, forceRefresh = false) {
    if (forceRefresh) {
      this.clearCache(patientId);
    }
    const reportDate = HisDate.sessionDate();
    const reportEndTs = this.resolveReportEndTs(reportDate);
    const [encountersRaw, labOrders, patient, enrollmentResult] = await Promise.all([
      this.getEncounters(patientId),
      this.getLabOrders(patientId),
      this.getPatient(patientId),
      this.getEnrollment(patientId)
    ]);
    const drugOrdersRaw = await this.getDrugOrders(patientId, encountersRaw);
    ...
    const encounters = this.isOffline() ? encountersRaw : selection.encounters;
    const encounterById = this.buildEncounterByIdIndex(encounters);
    const index = this.buildObsIndex(encounters);
    const presentTypes = new Set(Array.from(encounterById.values()).map((e) => e.encounterTypeId));
    const hasT = (t) => presentTypes.has(t);
    const ensure = (v) => {
    ...
    const enrollmentData = {
      ...
      apgarScoreAt1: ensure(this.getLatestValue(index, ["APGAR score at 1 minute", "Apgar score at 1 minute", "APGAR score at 1 min", "Apgar score at 1 min"])),
      apgarScoreAt5: ensure(this.getLatestValue(index, ["APGAR score at 5 minutes", "Apgar score at 5 minutes", "APGAR score at 5 min", "Apgar score at 5 min"])),
      apgarScoreAt10: ensure(this.getLatestValue(index, ["APGAR score at 10 minutes", "Apgar score at 10 minutes", "APGAR score at 10 min", "Apgar score at 10 min"])),
    ...
    const hasVitalsEncounter = hasT(EncounterTypeId.VITALS);
    const vitalsData = !hasVitalsEncounter ? null : {
    ...
    const triageData = !(hasT(EncounterTypeId.NEONATAL_TRIAGE) || hasT(EncounterTypeId.NEONATAL_EMERGENCY_TRIAGE)) ? null : {
      crying,
      breathing,
    ...
    const signsSymptomsData = !hasT(EncounterTypeId.NEONATAL_SIGNS_SYMPTOMS) ? null : {
    ...
    const reviewOfSystemsData = !hasT(EncounterTypeId.NEONATAL_REVIEW_OF_SYSTEMS) ? null : Object.fromEntries(reviewOfSystemsDataEntries);
    ...
    const generalExamData = !hasT(EncounterTypeId.PHYSICAL_EXAMINATION_BABY) ? null : {
    ...
    const systemicExamData = !hasT(EncounterTypeId.NEONATAL_SYSTEMIC_EXAMINATION) ? null : {
    ...
    const finalClinicalReviewOutcomesData = !hasT(EncounterTypeId.NEONATAL_CLINICAL_REVIEW_OUTCOMES) ? null : {
    ...
    const diagnosisData = !hasT(EncounterTypeId.DIAGNOSIS) ? null : {
    ...
    const treatmentPlanData = !(hasT(EncounterTypeId.TREATMENT) || prescriptions.length > 0) ? null : {
      pharmacological: {
...
export { NeonatalReportService as N };
