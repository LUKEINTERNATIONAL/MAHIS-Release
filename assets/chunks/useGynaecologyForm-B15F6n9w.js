...
import { c as computed } from './vendor-OAxQVBFs.js';

const STI_OPTIONS = [
  { value: "Genital Ulcer Disease (GUD)", label: "Genital Ulcer Disease (GUD)" },
  { value: "Male Urethral Discharge Syndrome (MUDS)", label: "Male Urethral Discharge Syndrome (MUDS)" },
  ...
  { value: "Acute Scrotal Swelling (SS)", label: "Acute Scrotal Swelling (SS)" },
  { value: "Inguinal Bubo (BU)", label: "Inguinal Bubo (BU)" },
  { value: "Balanitis / Balanoposthitis (BA)", label: "Balanitis / Balanoposthitis (BA)" },
  { value: "Neonatal Conjunctivitis (NC)", label: "Neonatal Conjunctivitis (NC)" },
...
const getSTITreatmentFieldName = (sti) => `Treatment for ${sti}`;
  ...
  const gynaecologyFormSection = computed(() => {
    return [
      ...
      // ========== GYNAECOLOGICAL SURGERY ==========
      {
        ...
        required: false,
        validation: (value) => {
          if (value && value.length < 3) return "Gynaecological surgery must be at least 3 letters long";
          return null;
        },
        grid: { s: "12" }
      },
