import { c as computed, s as defineComponent, a2 as onMounted, n as nextTick, y as openBlock, O as createBlock, B as withCtx, C as createBaseVNode, A as createVNode, F as unref, a$ as personOutline, f as ref, K as modalController, w as watch, z as createElementBlock, bK as IonCard, bd as IonCardContent, a7 as IonLabel, a5 as createTextVNode, J as Fragment, R as renderList, a4 as normalizeClass, bb as IonCardHeader, ba as IonCardTitle, L as IonIcon, D as toDisplayString, H as createCommentVNode, Q as alertCircleOutline, by as IonText, N as IonButton, e3 as createOutline, bG as addOutline } from './vendor-C-pvji42.js';
import { y as StandardValidations, K as ObservationService, b as EncounterTypeId, z as StandardForm, F as DynamicButton, r as StandardModal, t as toastWarning, H as HisDate, G as toastSuccess, u as useDemographicsStore, o as createModal, b5 as RelationshipService, J as savePatientRecord, _ as _export_sfc, n as icons, a6 as useUserStore } from '../index-W56NDnhc.js';
import { F as FindRegisterPatient } from './FindRegisterPatient-9InpmZ30.js';
  ...
  const newbornDetailsFormSection = computed(() => {
    return [
      ...
      },
      {
        ...
        grid: { s: "12" },
        condition: (allFormValues) => {
          const birthWeight = parseInt(allFormValues["Head circumference"]);
          return birthWeight > 20 && birthWeight < 33;
        }
      },
      {
        componentType: "Alert",
        ...
        condition: (allFormValues) => {
          const birthWeight = parseInt(allFormValues["Head circumference"]);
          return birthWeight > 35 && birthWeight <= 45;
        }
      },
      {
        componentType: "Dashes"
...
const _sfc_main$3 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    const openBabyModal = async (index) => {
      const babyData = await createModal(_sfc_main$4, { class: "large-medium-width-modal" }, true, {
        babyIndex: index,
...
const DeliveryNewbornDetails = /* @__PURE__ */ _export_sfc(_sfc_main$3, [["__scopeId", "data-v-b9612588"]]);
...
const useOtherDeliveryDetailsForm = () => {
  ...
  const otherDeliveryDetailsFormSection = computed(() => {
    const hasObstetricComplicationSelected = (formValues) => {
      const complication = formValues?.["Obstetric complications"];
      return !!complication && complication !== "None";
    };
    const getSelectedCareNames = (formValues) => {
      const selected = formValues?.["Obstetric care provided"];
      if (!selected) return [];
      if (Array.isArray(selected)) {
        return selected.map((item) => String(item?.name ?? item?.label ?? item?.value ?? item));
      }
      return [String(selected?.name ?? selected?.label ?? selected?.value ?? selected)];
    };
    const hasCareOption = (formValues, option) => {
      return getSelectedCareNames(formValues).includes(option);
    };
    return [
      {
        componentType: "radioButtonField",
      ...
      },
      {
        ...
        obsValueType: "value_coded",
        condition: (formValues) => {
          return hasObstetricComplicationSelected(formValues);
        },
        options: [
          { label: "None", value: "None", exclusive: true },
        ...
        validation: (value, formValues) => {
          if (!hasObstetricComplicationSelected(formValues)) return null;
          if (!Array.isArray(value) || value.length === 0) {
            return "Obstetric care provided is required";
          }
          const selectedNames = getSelectedCareNames(formValues);
          const hasNone = selectedNames.includes("None");
          const hasOtherOptions = selectedNames.some((item) => item !== "None");
          if (hasNone && hasOtherOptions) {
            return "'None' cannot be selected with other options";
          }
          return null;
        }
      },
      ...
      //         return hasCareOption(formValues, "Evacuation");
      //     }
      // },
      {
        componentType: "radioButtonField",
        ...
        validation: (value, formValues) => {
          if (!hasCareOption(formValues, "Evacuation")) return null;
      ...
      },
      {
        ...
        validation: (value, formValues) => {
          if (formValues["IV Fluids"] !== "Yes") return null;
          return StandardValidations.required(value);
        }
      },
      {
      ...
      },
      {
        ...
        validation: (value, formValues) => {
          if (formValues["Type of fluids"] !== "Other, specify") return null;
          return StandardValidations.required(value);
        }
      },
      // Blood transfusion
      ...
      },
      {
        ...
        validation: (value, formValues) => {
          if (formValues["Blood transfusion"] !== "Yes") return null;
          return StandardValidations.required(value);
        }
      },
      {
      ...
      },
      {
        ...
        validation: (value, formValues) => {
          if (formValues["Blood transfusion"] !== "Yes") return null;
          return StandardValidations.required(value) || StandardValidations.isNotEmptyandNumber(value);
        }
      },
      {
        componentType: "Dashes",
      ...
      },
      {
        ...
        obsValueType: "value_text",
        condition: (formValues) => {
          return hasCareOption(formValues, "Other");
        },
        grid: { xs: "12", sm: "6" },
        validation: (value, formValues) => {
          if (!hasCareOption(formValues, "Other")) return null;
          return StandardValidations.required(value);
        }
      }
    ];
  ...
  setup(__props, { expose: __expose }) {
    ...
    const onSubmit = async () => {
      ...
      if (await ObservationService.buildSaveObs(data, EncounterTypeId.Labour_and_delivery_visit))
        toastSuccess("Other delivery details data saved successfully");
      formRef.value?.resetForm();
    };
    __expose({
      onSubmit,
        ...
        createVNode(unref(IonCard), { class: "section" }, {
          default: withCtx(() => [
            createVNode(unref(IonCardContent), null, {
              default: withCtx(() => [
                createVNode(StandardForm, {
                  subtitle: "Other Delivery Details",
                  ref_key: "formRef",
                  ref: formRef,
                  "form-data": deliveryDetailsNewbornDetailsForm.value
                }, null, 8, ["form-data"])
...
const OtherDeliveryDetails = /* @__PURE__ */ _export_sfc(_sfc_main$2, [["__scopeId", "data-v-0ca3288f"]]);
...
const SecondStageDelivery = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["__scopeId", "data-v-da2eab5a"]]);

const optionalNumericInRange = (min, max) => {
  return (value) => {
    if (value === "" || value == null || value === void 0) return null;
    const numErr = StandardValidations.isNotEmptyandNumber(value);
    if (numErr) return numErr;
    return StandardValidations.checkMinMax(value, min, max);
  };
};
  ...
  const thirdStageDeliveryFormSection = computed(() => {
    return [
      ...
      },
      {
        ...
        type: "number",
        unit: "g",
        validation: optionalNumericInRange(100, 3e3),
        grid: { xs: "12", sm: "6" }
      },
      {
        componentType: "inputField",
        ...
        type: "number",
        unit: "cm",
        validation: optionalNumericInRange(1, 50),
        grid: { xs: "12", sm: "6" }
      },
      {
        componentType: "Dashes",
      ...
      },
      {
        ...
        type: "number",
        unit: "cm",
        validation: optionalNumericInRange(1, 200),
        grid: { xs: "12", sm: "6" }
      },
      {
        componentType: "Dashes",
      ...
      },
      {
        ...
        obsValueType: "value_numeric",
        type: "number",
        validation: optionalNumericInRange(1, 3),
        grid: { xs: "12" }
      },
      {
        componentType: "Dashes",
