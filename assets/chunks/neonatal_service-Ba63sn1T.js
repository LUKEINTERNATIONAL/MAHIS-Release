import { cJ as __vitePreload } from './vendor-QOXikrLM.js';
import { S as Service, u as useDemographicsStore, J as savePatientRecord, K as ObservationService, b as EncounterTypeId, H as HisDate, al as ProgramService, M as ProgramId, aZ as AppEncounterService, N as ConceptService, a8 as useUserStore } from '../index-SKNIAGXC.js';
...
class NeonatalService extends Service {
  static async saveOfflineEncounterToPatientRecord(params) {
    const demographicsStore = useDemographicsStore();
    const patientData = JSON.parse(JSON.stringify(demographicsStore.patient));
    (patientData[params.key] ??= {}).unsaved ??= [];
    const entry = {
      patient_id: params.patientId,
      encounter_type_id: params.encounterTypeId,
      encounter_type_name: params.encounterTypeName,
      encounter_datetime: params.date,
      observations: params.observations,
      ...params.extra || {}
    };
    if (params.encounterId) {
      entry.encounter_id = params.encounterId;
    }
    patientData[params.key].unsaved.push(entry);
  ...
  static async enrollNeonate(patientId, enrollmentData, date = HisDate.sessionDate()) {
    ...
    const data = await ObservationService.addObsToEncounterPatient(observations, EncounterTypeId.NEONATAL_ENROLLMENT);
  ...
  static async buildEnrollmentObservations(data, date) {
    ...
    await pushCoded("Mother on ART", data.motherOnArt);
    ...
    await pushCoded("Baby given 2P", data.babyGiven2p);
    ...
    await pushCoded("Mother tested for Hepatitis", data.motherTestedForHepatitis);
  ...
  static async getPatientSummary(patientId, date = HisDate.sessionDate()) {
    if (Service.getPouchDbStatus() || Service.getLanConnectionStatus()) {
      console.log("[NeonatalService] Using offline patient summary (PouchDB/LAN mode)");
      const { default: NeonatalOfflineService } = await __vitePreload(async () => { const { default: NeonatalOfflineService } = await import('./neonatal_offline_service-DeAogKIw.js');return { default: NeonatalOfflineService }},true              ?[]:void 0);
      return await NeonatalOfflineService.getPatientSummary(String(patientId));
    } else {
  ...
  static async getSavedEncounters(patientId, date = HisDate.sessionDate()) {
    if (Service.getPouchDbStatus() || Service.getLanConnectionStatus()) {
      console.log("[NeonatalService] Using offline saved encounters (PouchDB/LAN mode)");
      const { default: NeonatalOfflineService } = await __vitePreload(async () => { const { default: NeonatalOfflineService } = await import('./neonatal_offline_service-DeAogKIw.js');return { default: NeonatalOfflineService }},true              ?[]:void 0);
      return await NeonatalOfflineService.getSavedEncounters(String(patientId));
    } else {
   ...
   */
  static async getStatistics(date = HisDate.sessionDate(), filter) {
    if (Service.getPouchDbStatus() || Service.getLanConnectionStatus()) {
      console.log("[NeonatalService] Using offline statistics (PouchDB/LAN mode)");
      const { default: NeonatalOfflineService } = await __vitePreload(async () => { const { default: NeonatalOfflineService } = await import('./neonatal_offline_service-DeAogKIw.js');return { default: NeonatalOfflineService }},true              ?[]:void 0);
      const locationId = Service.getUserLocationId();
      return await NeonatalOfflineService.getStatistics(locationId, filter);
    } else {
      return this.getJson("/neonatal/statistics", { date, filter });
    }
  }
  /**
  ...
  static async exitProgram(patientId, exitData) {
    ...
    if (Service.getPouchDbStatus() || Service.getLanConnectionStatus()) {
      return await this.saveOfflineEncounterToPatientRecord({
        patientId,
        key: "neonatal_program_exit",
        encounterTypeId: EncounterTypeId.NEONATAL_DISCHARGE,
        encounterTypeName: "NEONATAL PROGRAM EXIT",
        observations: [],
        date: exitData?.date_completed || HisDate.sessionDate(),
        extra: { outcome: exitData?.outcome, date_completed: exitData?.date_completed }
      });
    } else {
      return this.postJson("/neonatal/exit", payload);
    }
  }
  /**
  ...
  static async saveSystemicExaminationAssessment(patientId, examinationData, date = HisDate.sessionDate()) {
    try {
      ...
      const data = await ObservationService.addObsToEncounterPatient(observations, EncounterTypeId.NEONATAL_SYSTEMIC_EXAMINATION);
      await savePatientRecord(data);
  ...
  static async saveSignsSymptomsAssessment(patientId, signsData, date = HisDate.sessionDate()) {
    try {
      ...
      const data = await ObservationService.addObsToEncounterPatient(observations, EncounterTypeId.NEONATAL_SIGNS_SYMPTOMS);
      await savePatientRecord(data);
  ...
  static async buildSignsSymptomsObservations(data, date) {
    ...
    if (hasValue(data.presenting_complaints_other)) {
      await pushText("Other sign and symptom", data.presenting_complaints_other);
    }
  ...
  static async saveGeneralExaminationAssessment(patientId, data, date = HisDate.sessionDate()) {
    try {
      console.log("[NeonatalService] saveGeneralExaminationAssessment called with data:", data);
      ...
      if (observations.length === 0) {
        ...
        console.warn("[NeonatalService] Form data received:", JSON.stringify(data, null, 2));
        return null;
      }
      ...
      const patientRecord = await ObservationService.addObsToEncounterPatient(observations, EncounterTypeId.PHYSICAL_EXAMINATION_BABY);
  ...
  static async saveReviewOfSystemsAssessment(patientId, rosData, date = HisDate.sessionDate()) {
    try {
      ...
      const data = await ObservationService.addObsToEncounterPatient(observations, EncounterTypeId.NEONATAL_REVIEW_OF_SYSTEMS);
      await savePatientRecord(data);
  ...
  static async getWeightTrendSinceBirth(patientId) {
    ...
    const getLatestObsByConcept = (observations2, conceptCandidates) => {
      const candidates = conceptCandidates.map(normalizeName);
      const matches = observations2.filter((obs) => {
    ...
    const weightKg = parseWeightToKg(currentRaw);
    const accumulated = {
      birthWeightKg,
      weightKg,
    ...
    if (accumulated.birthWeightKg > 0 && accumulated.weightKg > 0) {
      const rawLoss = (accumulated.birthWeightKg - accumulated.weightKg) * 100 / accumulated.birthWeightKg;
      const normalizedLoss = Number.isFinite(rawLoss) ? Math.max(rawLoss, 0) : 0;
      accumulated.weight_loss_percentage = Number.isFinite(normalizedLoss) ? Math.round(normalizedLoss * 10) / 10 : null;
      if (normalizedLoss > 0) {
        accumulated.weight_loss_category = normalizedLoss >= 10 ? "weight_loss_above_10" : "weight_loss_below_10";
      } else {
        accumulated.weight_loss_category = "weight_loss_none";
      }
    } else {
      accumulated.weight_loss_percentage = null;
  ...
  static async saveVitalsAssessment(patientId, vitalsData, date = HisDate.sessionDate()) {
    try {
      ...
      const data = await ObservationService.addObsToEncounterPatient(observations, EncounterTypeId.VITALS);
      await savePatientRecord(data);
  ...
  static async buildReviewOfSystemsObservations(data, date) {
    ...
    await pushCoded("Has the baby passed stool", data.has_baby_passed_stool);
    ...
    await pushText("Other color of stools", data.other_color_of_stools);
    ...
    await pushCoded("Is the baby catheterized?", data.is_baby_catheterized);
  ...
  static async saveClinicalReviewOutcomes(patientId, outcomesData, date = HisDate.sessionDate()) {
    try {
      ...
      const data = await ObservationService.addObsToEncounterPatient(observations, EncounterTypeId.NEONATAL_CLINICAL_REVIEW_OUTCOMES);
      await savePatientRecord(data);
  ...
  static async dischargeNeonate(patientId, dischargeData, date = HisDate.sessionDate()) {
    try {
      ...
      await encounter.createEncounter();
      if (observations.length > 0) {
        const patientRecord = await ObservationService.addObsToEncounterPatient(observations, EncounterTypeId.NEONATAL_DISCHARGE);
        await savePatientRecord(patientRecord);
      }
      ...
      return { success: true, encounterId: null };
    } catch (error) {
