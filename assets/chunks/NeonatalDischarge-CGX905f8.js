import { s as defineComponent, a3 as onMounted, y as openBlock, z as createElementBlock, C as createBaseVNode, Q as normalizeClass, A as createVNode, F as unref, d3 as person, L as IonIcon, D as toDisplayString, f as ref, c as computed, h as inject, w as watch, O as createBlock, M as IonSpinner, R as alertCircleOutline, N as IonButton, B as withCtx, a5 as createTextVNode, a$ as medkitOutline, J as Fragment, S as renderList, H as createCommentVNode, f0 as chevronUpOutline, a9 as chevronDownOutline, a4 as onUnmounted, aT as medicalOutline, b9 as calendarOutline, bW as trashOutline, bC as createStaticVNode, ab as checkmarkOutline, x as resolveComponent, aH as IonContent, bw as IonPage, aM as useRouter, bN as useRoute, b2 as printOutline, f1 as provide } from './vendor-NYNX75Tj.js';
import { s as storeToRefs } from './pinia-Byno_K7k.js';
import { u as useDemographicsStore, H as HisDate, P as PatientService, a8 as RelationsService, _ as _export_sfc, z as StandardForm, bH as DrugOrderService, ck as Banner, cn as SearchableDropdown, co as DateInputField, T as Toolbar, t as toastWarning, G as toastSuccess, x as toastDanger } from '../index-DRy1cgfg.js';
import { N as NeonatalService } from './neonatal_service-BBBJowR5.js';
import { D as DemographicBar } from './DemographicBar-45M9jSpH.js';
import { z as getDischargeSectionByName, A as neonatalDischargeFormKey, B as neonatalDischargeSections, b as neonatalClinicalReviewOutcomesFormKey, N as NeonatalStepper, C as useNeonatalDischargeStore, D as getDischargeSectionIndexByName } from './NeonatalStepper-B8K93umU.js';
import { I as IMAGES } from './images-_va7dFSn.js';
import { u as useClinicalReviewSignOff, n as neonatalClinicalReviewSignOffSections, R as ReportFooter, a as ReportHeader, b as ReportRow, c as ReportSection } from './ReportFooter-BL26XvLn.js';
...
const _hoisted_3$6 = { class: "patient-section" };
const _hoisted_4$6 = { class: "patient-details" };
const _hoisted_5$4 = { class: "patient-name" };
const _hoisted_6$4 = { class: "meta-line" };
...
const _hoisted_11$3 = { class: "mother-section" };
const _hoisted_12$2 = { class: "mother-value" };
const _sfc_main$c = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const getObservationValue = (conceptCandidates) => {
      ...
      for (const obs of observations) {
        ...
        if (names.some((n) => normalizedCandidates.includes(n))) {
          matches.push(obs);
        }
      }
    ...
    const fetchMotherName = async () => {
      ...
      try {
        const obsMother = getObservationValue(["Mother name", "Mother's name", "Mother full name"]);
        if (obsMother) {
          const valueText = extractObsValue(obsMother);
          if (valueText) {
            motherName.value = valueText;
            return;
          }
        }
        const patientService = new PatientService();
        const relationships = await patientService.getGuardian();
        const relationTypesRaw = await RelationsService.getRelations();
        const relationTypes = Array.isArray(relationTypesRaw) ? relationTypesRaw : [];
        const relationTypeById = /* @__PURE__ */ new Map();
        relationTypes.forEach((t) => {
          const id = Number(t?.relationship_type_id ?? t?.relationshipTypeId ?? t?.id);
          if (Number.isFinite(id)) relationTypeById.set(id, t);
        });
        if (relationships && relationships.length > 0) {
          const motherRel = relationships.find((rel) => {
            const relType = rel.type ?? rel.relationship_type ?? relationTypeById.get(Number(rel?.relationship_type_id ?? rel?.relationshipTypeId ?? rel?.type_id));
            const type = relType?.b_is_to_a || relType?.bIsToA || "";
            return type === "Mother" || type === "Parent";
          });
          if (motherRel) {
            const nameRecord = motherRel?.relation?.names?.[0] ?? motherRel?.person?.names?.[0] ?? motherRel?.patient?.names?.[0] ?? motherRel?.relative?.names?.[0];
            if (nameRecord) {
              motherName.value = [nameRecord.given_name, nameRecord.middle_name, nameRecord.family_name].filter(Boolean).join(" ");
            }
          } else {
            motherName.value = "";
          }
        ...
        if (!motherName.value) motherName.value = "N/A";
      } catch (error) {
        ...
        createBaseVNode("div", _hoisted_2$7, [
          createBaseVNode("div", _hoisted_3$6, [
            ...
            createBaseVNode("div", _hoisted_4$6, [
              createBaseVNode("h2", _hoisted_5$4, toDisplayString(unref(patient)?.personInformation?.given_name) + " " + toDisplayString(unref(patient)?.personInformation?.family_name), 1),
              createBaseVNode("div", _hoisted_6$4, [
                createBaseVNode("span", null, toDisplayString(genderLabel.value), 1),
                _cache[0] || (_cache[0] = createBaseVNode("span", { class: "separator" }, "•", -1)),
                createBaseVNode("span", null, toDisplayString(ageLabel.value), 1),
                _cache[1] || (_cache[1] = createBaseVNode("span", { class: "separator" }, "•", -1)),
                createBaseVNode("span", null, toDisplayString(dobLabel.value), 1)
              ]),
              createBaseVNode("div", _hoisted_7$4, [
                _cache[2] || (_cache[2] = createBaseVNode("span", { class: "label" }, "MRN:", -1)),
                createBaseVNode("span", _hoisted_8$3, toDisplayString(unref(patient)?.ID), 1)
              ]),
              createBaseVNode("div", _hoisted_9$3, [
                _cache[3] || (_cache[3] = createBaseVNode("span", { class: "label" }, "Admission Date:", -1)),
                createBaseVNode("span", _hoisted_10$3, toDisplayString(admissionDate.value || "--"), 1)
              ])
            ])
          ]),
          _cache[5] || (_cache[5] = createBaseVNode("div", { class: "vertical-divider" }, null, -1)),
          createBaseVNode("div", _hoisted_11$3, [
            _cache[4] || (_cache[4] = createBaseVNode("div", { class: "mother-label" }, "Mother's Full Name", -1)),
            createBaseVNode("div", _hoisted_12$2, toDisplayString(motherName.value || "Searching..."), 1)
          ])
        ])
      ]);
    };
...
const ProfileInformation = /* @__PURE__ */ _export_sfc(_sfc_main$c, [["__scopeId", "data-v-c176104d"]]);
...
const _hoisted_8$2 = { class: "weight-content" };
const _hoisted_9$2 = { class: "weight-value" };
const _hoisted_10$2 = { class: "weight-date" };
const _hoisted_11$2 = { class: "section-subtitle" };
const _sfc_main$b = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    const hydrateVitalsFromObservations = () => {
      try {
        const allObs = collectAllObservations();
        const isEnrollmentObs = (obs) => {
          const name = obs?.encounter_type_name ?? obs?.encounterTypeName ?? obs?.encounter?.encounter_type_name ?? obs?.encounter?.encounterTypeName ?? obs?.encounter_name ?? obs?.encounterName ?? "";
          return String(name).toUpperCase().includes("ENROLL");
        };
        allObs.forEach((obs) => {
          if (!isEnrollmentObs(obs)) return;
          const labels = getConceptNamesFromObs(obs);
          const value = obs?.value_text ?? obs?.value_numeric ?? obs?.value_datetime ?? obs?.value ?? obs?.value_coded_name ?? obs?.value_coded ?? obs?.answer;
          console.log("[DischargeVitals][Enrollment]", { labels, value, obs });
        });
        const hivObs = getObservationValue(["Mother HIV status", "HIV status", "Mother HIV Status"]);
        if (hivObs) {
        ...
        const weightObs = getObservationValue(["Birth weight", "Birth weight (kg)", "Birth weight (g)", "Birth weight (grams)"]);
        if (weightObs) {
          const raw = weightObs.value_numeric ?? weightObs.value_text ?? weightObs.value;
          const num = Number(raw);
          if (Number.isFinite(num)) {
            const kg = num > 20 ? num / 1e3 : num;
            birthWeight.value = kg.toString();
          } else {
            birthWeight.value = "";
          }
            ...
            createBaseVNode("div", _hoisted_5$3, [
              ...
              createBaseVNode("div", _hoisted_6$3, [
                ...
                createBaseVNode("div", _hoisted_8$2, [
                  createBaseVNode("span", _hoisted_9$2, toDisplayString(birthWeight.value ? birthWeight.value + " kg" : "..."), 1),
                  createBaseVNode("span", _hoisted_10$2, "Date: " + toDisplayString(birthDate.value || "..."), 1)
                ])
              ])
            ])
          ]),
          createBaseVNode("div", _hoisted_11$2, toDisplayString(unref(sectionConfig).subtitle), 1)
        ]),
...
const DischargeVitals = /* @__PURE__ */ _export_sfc(_sfc_main$b, [["__scopeId", "data-v-c9dd557e"]]);
...
const _sfc_main$6 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    const infoBannerConfig = {
      ...
      message: "You can assign clinic days for the baby.",
      noMargin: true
    };
            ...
            return openBlock(), createElementBlock("div", {
              ...
              createBaseVNode("div", _hoisted_3$3, [
                ...
                createBaseVNode("div", _hoisted_6$1, [
                  createBaseVNode("span", null, toDisplayString(appt.date ? unref(HisDate).toStandardHisDisplayFormat(appt.date) : ""), 1),
                  createVNode(unref(IonIcon), {
...
const ClinicFollowUp = /* @__PURE__ */ _export_sfc(_sfc_main$6, [["__scopeId", "data-v-02588b56"]]);
...
const _sfc_main$5 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    const infoBannerConfig = {
      componentType: "Banner",
      message: "Please educate the mother on the following:",
      noMargin: true
    };
    const formRef = ref(null);
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("div", _hoisted_1$4, [
        createVNode(Banner, { config: infoBannerConfig }),
        _cache[0] || (_cache[0] = createStaticVNode('<div class="education-section" data-v-a0231697><h3 class="section-title" data-v-a0231697>1. Danger Signs</h3><ul class="education-list" data-v-a0231697><li data-v-a0231697>Jaundice</li><li data-v-a0231697>Convulsions</li><li data-v-a0231697>Not able to breast feed</li><li data-v-a0231697>Difficulties in breathing</li><li data-v-a0231697>Lethargic</li><li data-v-a0231697>Fever</li><li data-v-a0231697>Hypothermia</li><li data-v-a0231697>Abdominal distention</li><li data-v-a0231697>Bleeding from the cord and what to do if any</li></ul></div><div class="education-section" data-v-a0231697><h3 class="section-title" data-v-a0231697>1. GENERAL EDUCATION</h3><ul class="education-list" data-v-a0231697><li data-v-a0231697>Immunization</li><li data-v-a0231697>Family planning</li><li data-v-a0231697>Hygiene</li><li data-v-a0231697>Keeping baby warm</li><li data-v-a0231697>Malaria Prevention</li></ul></div>', 2)),
        createVNode(StandardForm, {
          formData: unref(formData),
          ref_key: "formRef",
          ref: formRef
...
const HealthEducation = /* @__PURE__ */ _export_sfc(_sfc_main$5, [["__scopeId", "data-v-a0231697"]]);
...
const SignOff = /* @__PURE__ */ _export_sfc(_sfc_main$3, [["__scopeId", "data-v-72a8a248"]]);
  ...
  setup(__props, { expose: __expose }) {
    ...
    const formData = computed(() => {
      ...
      return [
        {
          componentType: "Banner",
          name: "dischargeSummaryReviewReminder",
          message: "Please review your entries. You can go back to any previous section to make corrections if necessary before proceeding."
        },
        {
...
const NeonatalDischarge = /* @__PURE__ */ _export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-57baac2a"]]);
