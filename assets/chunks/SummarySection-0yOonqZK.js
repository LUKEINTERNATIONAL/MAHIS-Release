import { cj as Banner, ck as SummaryField, _ as _export_sfc } from '../index-B8ztKi9t.js';
import { n as neonatalReviewOfSystemsFormKey, a as neonatalSignsSymptomsFormKey, b as neonatalClinicalReviewOutcomesFormKey, c as neonatalTriageFormKey, d as neonatalSystemicExaminationFormKey, e as neonatalTreatmentPlanFormKey, f as neonatalReviewOfSystemsSections, g as neonatalSignsSymptomsSections, h as neonatalEnrollmentSections, i as neonatalClinicalReviewOutcomesSections, j as neonatalSystemicExaminationSections, k as neonatalTreatmentPlanSections, l as neonatalInvestigationSections } from './NeonatalStepper-B723xsv1.js';
import { n as neonatalEnrollmentFormKey } from './useNeonatalEnrollmentStore-C7DTMepN.js';
import { u as useNeonatalTriageStore, n as neonatalTriageSections } from './triage-CrdX2dj8.js';
import { a as neonatalGeneralExaminationFormKey, n as neonatalGeneralExaminationSections } from './neonatal_service-QHKCjCoH.js';
import { s as defineComponent, h as inject, i as isRef, y as openBlock, z as createElementBlock, A as createVNode, O as createBlock, H as createCommentVNode, c as computed } from './vendor-DvGYa1v4.js';
  ...
  for (let sectionIndex = 0; sectionIndex < limit; sectionIndex += 1) {
    ...
    for (const field of sectionFields) {
      ...
      if (typeof fieldCondition === "function") {
        try {
          if (!fieldCondition(allFormValues)) continue;
        } catch {
        }
      }
      const name = field.name;
...
const neonatalInvestigationFormKey = Symbol("neonatal-investigation-form");
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    const investigationForm = inject(neonatalInvestigationFormKey, void 0);
    ...
    const sectionsConfig = computed(() => {
      ...
      if (clinicalReviewOutcomesForm) {
        return neonatalClinicalReviewOutcomesSections;
      }
      if (triageForm) {
      ...
      if (investigationForm) {
        return neonatalInvestigationSections;
      }
      return [];
    });
    ...
    const sectionBannerConfig = computed(
      () => sectionConfig.value.formData.find((field) => field.componentType === "Banner")
    );
    const displayBannerConfig = computed(() => sectionBannerConfig.value || reviewBannerConfig);
    ...
    const allFormValues = computed(() => {
      ...
      if (treatmentPlanForm) {
        ...
        if (Array.isArray(snapshot.prescriptions)) {
          snapshot.prescriptions = [...snapshot.prescriptions];
        }
        if (Array.isArray(snapshot.treatments)) {
          snapshot.treatments = [...snapshot.treatments];
        }
        if (Array.isArray(snapshot.thermalCareDetails)) {
          snapshot.thermalCareDetails = [...snapshot.thermalCareDetails];
        }
        if (Array.isArray(snapshot.feedingSupportDetails)) {
          snapshot.feedingSupportDetails = [...snapshot.feedingSupportDetails];
        }
        if (Array.isArray(snapshot.oxygenTherapyDetails)) {
          snapshot.oxygenTherapyDetails = [...snapshot.oxygenTherapyDetails];
        }
        return snapshot;
      }
      return { ...rosForm || signsSymptomsForm || enrollmentForm || clinicalReviewOutcomesForm || investigationForm || {} };
    });
            ...
            summaryRows: [
              {
                ...
                label: "Summary",
                value: "No investigations ordered."
              }
            ]
          };
        }
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("div", _hoisted_1, [
        createVNode(Banner, { config: displayBannerConfig.value }, null, 8, ["config"]),
...
const SummarySection = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-d5db99e0"]]);

export { SummarySection as S, neonatalDiagnosisFormKey as a, neonatalInvestigationFormKey as b, neonatalDiagnosisSections as n };
