import { s as defineComponent, h as inject, w as watch, y as openBlock, z as createElementBlock, A as createVNode, F as unref, f as ref, M as IonSpinner, a3 as onMounted, x as resolveComponent, C as createBaseVNode, J as Fragment, B as withCtx, O as createBlock, H as createCommentVNode, S as renderList, L as IonIcon, N as IonButton, aI as IonContent, bx as IonPage, aN as useRouter, bO as useRoute, b3 as printOutline, f3 as provide, a5 as createTextVNode } from './vendor-QOXikrLM.js';
import { s as storeToRefs } from './pinia-BRP2SQHv.js';
import { z as StandardForm, _ as _export_sfc, T as Toolbar, u as useDemographicsStore, K as ObservationService, b as EncounterTypeId, J as savePatientRecord, G as toastSuccess } from '../index-Biv93APm.js';
import { b as neonatalClinicalReviewOutcomesFormKey, N as NeonatalStepper, O as useNeonatalClinicalReviewOutcomesStore } from './NeonatalStepper-BknGWmDD.js';
import { C as ClinicalReviewProgressBar } from './ClinicalReviewProgressBar-ZRmExjH0.js';
import { n as neonatalClinicalReviewSignOffSections, u as useClinicalReviewSignOff, R as ReportFooter, a as ReportHeader, b as ReportRow, c as ReportSection } from './ReportFooter-DeiZb38H.js';
        ...
        createBaseVNode("div", _hoisted_3$1, [
          ...
          createBaseVNode("div", _hoisted_10, [
            ...
            createBaseVNode("div", _hoisted_12, [
              _ctx.reportData.vitalsData ? (openBlock(), createBlock(_component_ReportSection, {
                key: 0,
                title: "Vital Signs"
              }, {
                default: withCtx(() => [
...
const ClinicalReviewReport = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["render", _sfc_render$1], ["__scopeId", "data-v-323186aa"]]);
  ...
  setup() {
    ...
    const getSaveFunction = (currentIndex) => {
      if (currentIndex === finalStepIndex) {
        return async () => {
          console.log("[ClinicalReviewSignOff] Save function called");
          if (isSaving.value) {
            console.log("[ClinicalReviewSignOff] Already saving, returning");
            return;
          }
          if (patientId == null) {
            console.error("[ClinicalReviewSignOff] No patient ID available");
            return;
          }
          console.log("[ClinicalReviewSignOff] Starting save for patient:", patientId);
          isSaving.value = true;
          try {
            const formData = clinicalReviewOutcomesFormData;
            const signOffData = {
              healthcareWorkerId: formData.healthcareWorkerId || "",
              electronicSignature: formData.electronicSignature || "",
              userRole: formData.userRole || "",
              signOffDate: formData.signOffDate || ""
            };
            const signoffNotes = `[NEONATAL_SIGNOFF]${JSON.stringify(signOffData)}`;
            const observations = [
              await ObservationService.buildValueText("Clinician notes", signoffNotes),
              await ObservationService.buildValueText("Healthcare worker ID", signOffData.healthcareWorkerId),
              await ObservationService.buildValueText("Healthcare worker role", signOffData.userRole),
              await ObservationService.buildValueText("Electronic signature", signOffData.electronicSignature),
              await ObservationService.buildValueText("Sign-off date time", signOffData.signOffDate)
            ];
            if (formData.clinicalReviewOutcome) {
              observations.push(await ObservationService.buildValueCoded("Clinical review outcome", formData.clinicalReviewOutcome));
            }
            if (formData.clinicalReviewOutcomeOther) {
              observations.push(await ObservationService.buildValueText("Clinical review outcome other", formData.clinicalReviewOutcomeOther));
            }
            if (formData.safeguardConcerns) {
              observations.push(await ObservationService.buildValueText("Safeguard concerns", formData.safeguardConcerns));
            }
            console.log("[ClinicalReviewSignOff] Built observations:", observations.length);
            ...
            console.log("[ClinicalReviewSignOff] Encounter data prepared");
            await savePatientRecord(data);
            console.log("[ClinicalReviewSignOff] Patient record saved");
            NeonatalReportService.clearCacheForPatient(patientId);
            toastSuccess("Clinical review signed off successfully");
            showPrintReport.value = true;
            console.log("[ClinicalReviewSignOff] Save completed successfully");
          } catch (error) {
            console.error("[ClinicalReviewSignOff] Save failed with error:", error);
            console.error("[ClinicalReviewSignOff] Error details:", error instanceof Error ? error.message : String(error));
            toastSuccess("Clinical review signed off successfully");
            showPrintReport.value = true;
          } finally {
    ...
    const handlePrintReport = async () => {
      if (!patientId) return;
      await NeonatalReportService.generateClinicalReviewReport(patientId, printableContainer.value || void 0);
    };
    const finishClinicalReview = () => {
      if (patientId != null) {
        clinicalReviewOutcomesStore.clearClinicalReviewOutcomesForm(patientId);
      }
...
const ClinicalReviewSignOff = /* @__PURE__ */ _export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-30207fac"]]);
