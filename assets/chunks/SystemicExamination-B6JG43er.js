import { s as defineComponent, h as inject, w as watch, y as openBlock, O as createBlock, f as ref, c as computed, bx as IonPage, aI as IonContent, aN as useRouter, f3 as provide, fB as onIonViewWillLeave, x as resolveComponent, B as withCtx, A as createVNode } from './vendor-QOXikrLM.js';
import { s as storeToRefs } from './pinia-BRP2SQHv.js';
import { u as useDemographicsStore, z as StandardForm, T as Toolbar, t as toastWarning, G as toastSuccess, x as toastDanger, _ as _export_sfc } from '../index-SKNIAGXC.js';
import { d as neonatalSystemicExaminationFormKey, I as getSystemicExaminationSectionById, N as NeonatalStepper, J as useSystemicExaminationStore, k as neonatalSystemicExaminationSections } from './NeonatalStepper-DJUdVdam.js';
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    const gender = computed(() => {
      ...
      if (typeof g === "string") return g.toUpperCase().startsWith("M") ? "M" : g.toUpperCase().startsWith("F") ? "F" : "U";
      const val = g?.value;
    ...
    const withGenderDisabled = (options) => {
      ...
      return options.map((opt) => {
        ...
        const isMaleOpt = /male/i.test(label) && !(/female/i.test(label) || /female/i.test(value));
        const isFemaleOpt = /female/i.test(label) || /female/i.test(value);
        let disabled = false;
        if (gender.value === "M") {
    ...
    const formData = computed(() => {
      ...
      return fields.map((field) => {
        if (field.componentType === "infographicField" || field.componentType === "Banner") {
          return field;
        }
        ...
        if (field.name === "genitaliaFindings" && Array.isArray(field.options)) {
          return {
            ...field,
            initialValue,
    ...
    watch(
      gender,
      () => {
        const current = formRef.value?.getFormValues?.()?.genitaliaFindings;
        if (!current) return;
        const options = (formData.value || []).find((f) => f.name === "genitaliaFindings")?.options || [];
        const selected = options.find((opt) => opt.value === current);
        if (selected?.disabled) {
          formRef.value?.setFormValue?.("genitaliaFindings", null);
        }
      },
      { immediate: true }
    );
...
const SystemicExamination = /* @__PURE__ */ _export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-62163846"]]);
