...
import { c as computed } from './vendor-DOib3KBh.js';
...
const resetSTITreatmentValues = () => STI_OPTIONS.reduce(
  (fields, sti) => {
    ...
    return [
      ...
      },
      {
        ...
        onChange: (value) => {
          ...
          const updates = STI_OPTIONS.reduce(
            (fields, sti) => {
      ...
      {
        componentType: "inputField",
        header: "Other Gynecological information",
        name: "Other notes",
        icon: icons.editPen,
        required: false,
        grid: { s: "12" },
        rows: 4,
        validation: (value) => {
          if (isNaN(value)) return "Other gynecological information must be a text";
          return null;
        }
      }
    ];
