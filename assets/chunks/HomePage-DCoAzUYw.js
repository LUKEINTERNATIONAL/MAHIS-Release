...
const DASHBOARD_STATS_PATH$2 = "mnh/stats";
function getDashboardDate$2() {
  return HisDate.toStandardHisFormat(HisDate.sessionDate());
}
function mapResponseToDashboardData$2(raw) {
...
async function getAncDashboard() {
  const date = getDashboardDate$2();
  const programId = Service.getProgramID();
  try {
    const raw = await Service.getJson(DASHBOARD_STATS_PATH$2, {
      program_id: programId,
      date
    });
    if (!raw || typeof raw !== "object") return emptyDashboardData$2();
    return mapResponseToDashboardData$2(raw);
  } catch (_) {
    return emptyDashboardData$2();
  }
}
...
function percentOrZero$1(n) {
  return n != null && Number.isFinite(n) ? Math.round(Number(n) * 100) / 100 : 0;
}
function numOrZero$1(n) {
  return n != null && Number.isFinite(n) ? Number(n) : 0;
}
function mapResponseToDashboardData$1(raw) {
  const nonePct = percentOrZero$1(raw.obstetric_complication_none_percentage);
  const complicationBreakdown = [
    { label: "Postpartum haemorrhage", percentage: percentOrZero$1(raw.obstetric_complication_postpartum_haemorrhage_percentage) },
    { label: "Pre-eclampsia", percentage: percentOrZero$1(raw.obstetric_complication_pre_eclampsia_percentage) },
    { label: "Eclampsia", percentage: percentOrZero$1(raw.obstetric_complication_eclampsia_percentage) },
    { label: "Sepsis", percentage: percentOrZero$1(raw.obstetric_complication_sepsis_percentage) },
    { label: "Retained placenta", percentage: percentOrZero$1(raw.obstetric_complication_retained_placenta_percentage) },
    {
      label: "Perineal tear (2nd/3rd/4th)",
      percentage: percentOrZero$1(raw.obstetric_complication_perineal_tear_2nd_3rd_or_4th_degree_percentage)
    }
  ];
  return {
    totalDeliveriesWithStaffRecorded: numOrZero$1(raw.total_deliveries_with_staff_recorded),
    mothersDeliveredBySkilledAttendant: numOrZero$1(raw.mothers_delivered_by_skilled_attendant),
    percentageDeliveredBySkilledAttendants: percentOrZero$1(raw.percentage_delivered_by_skilled_attendants),
    clientsDeliveredAtHomeOrInTransit: numOrZero$1(raw.clients_delivered_at_home_or_in_transit),
    clientsDeliveredAtThisFacility: numOrZero$1(raw.clients_delivered_at_this_facility),
    totalDeliveriesWithPlaceRecorded: numOrZero$1(raw.total_deliveries_with_place_recorded),
    percentageDeliveredAtThisFacility: percentOrZero$1(raw.percentage_delivered_at_this_facility),
    caesareanSectionCount: numOrZero$1(raw.caesarean_section_count),
    totalDeliveriesWithModeRecorded: numOrZero$1(raw.total_deliveries_with_mode_recorded),
    percentageCaesareanSection: percentOrZero$1(raw.percentage_caesarean_section),
    percentageWithObstetricComplication: Math.round((100 - nonePct) * 100) / 100,
    complicationBreakdown,
    referralByCondition: (raw.referral_by_condition ?? []).map((r) => ({
      label: r.label ?? r.condition ?? "",
      percentage: percentOrZero$1(r.percentage)
    })),
    date: raw.date
  };
}
...
async function getLabourDashboard() {
  ...
  const programId = Service.getProgramID();
  try {
    if (Service.getLanConnectionStatus()) return emptyDashboardData$1();
    const raw = await Service.getJson(DASHBOARD_STATS_PATH$1, { program_id: programId, date });
    const payload = raw?.data && typeof raw.data === "object" ? raw.data : raw;
    if (!payload || typeof payload !== "object") return emptyDashboardData$1();
    return mapResponseToDashboardData$1(payload);
  } catch (_) {
...
const Dashboard$1 = /* @__PURE__ */ _export_sfc(_sfc_main$5, [["__scopeId", "data-v-2e017550"]]);
