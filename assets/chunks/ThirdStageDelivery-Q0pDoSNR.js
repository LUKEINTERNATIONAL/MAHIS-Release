import { c as computed, s as defineComponent, a3 as onMounted, n as nextTick, y as openBlock, O as createBlock, B as withCtx, C as createBaseVNode, A as createVNode, F as unref, b1 as personOutline, f as ref, K as modalController, w as watch, z as createElementBlock, bf as IonCardContent, a7 as IonLabel, a5 as createTextVNode, J as Fragment, S as renderList, bN as IonCard, Q as normalizeClass, bd as IonCardHeader, bc as IonCardTitle, L as IonIcon, D as toDisplayString, H as createCommentVNode, R as alertCircleOutline, bB as IonText, N as IonButton, e5 as createOutline, bJ as addOutline } from './vendor-8vOj_QAN.js';
import { y as StandardValidations, n as icons, K as ObservationService, b as EncounterTypeId, z as StandardForm, F as DynamicButton, r as StandardModal, t as toastWarning, H as HisDate, G as toastSuccess, u as useDemographicsStore, b7 as RelationshipService, J as savePatientRecord, _ as _export_sfc } from '../index-Bn2sysQf.js';
    ...
    return [
      ...
      },
      {
        ...
        condition: (allFormValues) => {
          const birthWeight = parseInt(allFormValues["Weight (grams)"]);
      ...
      },
      {
        ...
        validation: (value) => {
          ...
          if (!isNaN(Number(value))) return "Other mode of delivery must be a text";
          return null;
        },
      ...
      // Conditional: Other Notes
      {
        ...
        validation: (value) => {
          if (value === "" || value === null || value === void 0) return null;
          if (!isNaN(Number(value))) return "Value must be a text";
          if (value.length < 3) return "Value must be at least 3 characters long";
          return null;
        },
...
const _sfc_main$2 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    onMounted(async () => {
      await nextTick();
      const [deliveryDetails, vitals] = await Promise.all([
        ObservationService.getLatestObsByEncounterId(EncounterTypeId.Delivery_Details, props.baby),
        ObservationService.getLatestObsByEncounterId(EncounterTypeId.VITALS, props.baby)
      ]);
      const data = [...Array.isArray(deliveryDetails) ? deliveryDetails : [], ...Array.isArray(vitals) ? vitals : []];
      if (data.length > 0) {
        await nextTick();
        const groupedData = {};
        data.forEach((obs) => {
        ...
        Object.entries(groupedData).forEach(([fieldName, observations]) => {
          if (fieldName === "Newborn baby complications" || fieldName === "Management given to newborn") {
            ...
            if (obs.value_numeric !== void 0 && obs.value_numeric !== null) {
              if (fieldName === "Weight (grams)") {
                fieldValue = Number(obs.value_numeric).toString();
                formRef.value?.setFormValue("Weight (grams)", fieldValue);
                return;
              }
    ...
    const onSave = async () => {
      ...
      const patientRecord = await ObservationService.buildRelativeObs(babyRecord.value, formData, EncounterTypeId.Delivery_Details, props.deliveryDate);
      if (patientRecord && validationResult == null) {
        const weightGramsRaw = formData?.["Weight (grams)"];
        const heightCmRaw = formData?.["Height (cm)"];
        const weightGrams = weightGramsRaw !== void 0 && weightGramsRaw !== null && weightGramsRaw !== "" ? Number(weightGramsRaw) : NaN;
        const heightCm = heightCmRaw !== void 0 && heightCmRaw !== null && heightCmRaw !== "" ? Number(heightCmRaw) : NaN;
        const vitalsPayload = {};
        if (Number.isFinite(weightGrams)) vitalsPayload["Weight (grams)"] = weightGrams;
        if (Number.isFinite(heightCm)) vitalsPayload["Height (cm)"] = heightCm;
        if (Object.keys(vitalsPayload).length > 0) {
          vitalsPayload.obsValueType = {
            ...Number.isFinite(weightGrams) ? { "Weight (grams)": "value_numeric" } : {},
            ...Number.isFinite(heightCm) ? { "Height (cm)": "value_numeric" } : {}
          };
        }
        if (Object.keys(vitalsPayload).length > 0) {
          await ObservationService.buildRelativeObs(babyRecord.value, vitalsPayload, EncounterTypeId.VITALS, props.deliveryDate);
        }
  ...
  const deliveryNewbornDetailsFormSection = computed(() => {
    return [
      ...
      },
      {
        ...
        grid: { s: "10" },
        offset: "1",
        condition: (formValues) => formValues["Obstetric complications"] == "Other" || formValues["Obstetric complications"] == "Perineal tear"
      },
      {
        componentType: "radioButtonField",
        name: "Degree of Perineal tear",
        header: "Perineal tear",
        obsValueType: "value_coded",
        grid: { s: "10" },
        offset: "1",
        type: "inline",
        condition: (formValues) => !isStillbirth(formValues) && !isNeonatalDeath(formValues) && formValues["Obstetric complications"] == "Perineal tear",
        options: [
      ...
      },
      {
        ...
        grid: { s: "10" },
        offset: "2",
        condition: (formValues) => formValues["Obstetric complications"] == "Other"
      },
      {
        componentType: "Dashes",
        grid: { s: "11" },
      ...
      {
        componentType: "inputField",
        header: "Specify other obstetric care provided",
        name: "_obstetric care provided",
        obsValueType: "value_text",
        icon: icons.editPen,
        grid: { s: "10" },
        offset: "1",
        condition: (formValues) => formValues["Obstetric care provided"] == "Other"
      },
      {
        componentType: "Dashes",
        grid: { s: "10" },
      ...
      },
      {
        ...
        offset: "1",
        validation: (value) => {
          if (isNaN(value)) return "Volume must be a number";
          if (value < 0) return "Volume must be greater than 0";
          if (value > 5e3) return "Volume must be less than 1000";
          return null;
        },
        condition: (formValues) => {
          return !isStillbirth(formValues) && !isNeonatalDeath(formValues) && formValues["Blood transfusion"] === "Yes";
      ...
      },
      {
        ...
        validation: (value) => {
          if (value === "" || value === null || value === void 0) return null;
          if (!isNaN(Number(value))) return "Other evacuations must be a text";
          return null;
        },
        condition: (formValues) => {
          return !isStillbirth(formValues) && !isNeonatalDeath(formValues) && formValues["Obstetric care provided"]?.includes("Evacuation");
    ...
    return [
      ...
      },
      {
        ...
        validation: (value) => {
          ...
          if (value > 5e3) return "Estimated blood loss must be less than 5000";
          return null;
        }
      },
      {
        ...
        createVNode(unref(IonCard), { class: "section" }, {
          default: withCtx(() => [
            createVNode(unref(IonCardContent), null, {
              default: withCtx(() => [
                createVNode(StandardForm, {
                  subtitle: "Third stage of labour",
                  ref_key: "formRef",
                  ref: formRef,
...
const ThirdStageDelivery = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-37284227"]]);
