import { s as defineComponent, y as openBlock, O as createBlock, f as ref, c as computed, a2 as onMounted, z as createElementBlock, A as createVNode, B as withCtx, F as unref, bK as IonCard, J as Fragment, w as watch, bF as IonModal, I as IonHeader, aD as IonToolbar, aE as IonTitle, C as createBaseVNode, L as IonIcon, eO as eggOutline, a5 as createTextVNode, D as toDisplayString, be as IonButtons, N as IonButton, G as closeCircleOutline, a7 as IonLabel, aG as IonContent, H as createCommentVNode, bf as IonFooter, bH as saveOutline, R as renderList, a4 as normalizeClass, cn as checkmarkCircle, aL as useRouter, bX as chevronBackOutline, S as withDirectives, T as vShow, bu as IonPage } from './vendor-OSq5kYvl.js';
import { z as StandardForm, x as toastDanger, K as ObservationService, b as EncounterTypeId, G as toastSuccess, n as icons, t as toastWarning, _ as _export_sfc, u as useDemographicsStore, F as DynamicButton, C as useExposeFromStandardForm, S as Service, J as savePatientRecord, y as StandardValidations, T as Toolbar } from '../index-CbBGWx39.js';
import { D as DemographicBar } from './DemographicBar-DzqLjn6U.js';
import { _ as _sfc_main$i } from './Wizard.vue_vue_type_script_setup_true_lang-CNkcMDMw.js';
import { u as useFormWizard } from './useFormWizard-C_8e9DHP.js';
import { V as Vitals } from './Vitals-CLCbdeJi.js';
import { d as defineStore, s as storeToRefs } from './pinia-DzwNa_n5.js';
import { _ as _sfc_main$h } from './LevelOfConsciousness.vue_vue_type_script_setup_true_lang-CECZZJfv.js';
    ...
    const vaginalInspection = computed(() => {
      return [
        ...
        },
        {
          ...
          validation: validateAtLeastOne,
          onChange: (value) => {
            if (!value) return { "Other general exam findings": "" };
          }
        },
        {
          header: "Other general exam findings",
        ...
        },
        {
          ...
          obsValueType: "value_text",
          icon: icons.editPen,
          // Only show and validate if an inspection actually occurred
          condition: (allFormValues) => allFormValues["No vaginal inspection done"] !== true,
...
const _sfc_main$e = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
      ...
      return [
        {
          ...
          onChange: (value) => {
            if (value === "No") {
              return { "Digital": false, "Speculum": false, "Digital examination findings notes": "", "Speculum examination findings notes": "" };
            }
          }
        },
        {
          ...
          // Validation: If "Yes" is selected for the exam, at least one type (Digital or Speculum) should be checked
          validation: (value, allFormValues) => {
    ...
    const onSubmit = async () => {
      ...
      const formValues = formRef.value?.getFormValues() || {};
      if (formValues["Is vaginal examination done?"]) {
        try {
          await ObservationService.buildAddObsToEncounterPatient(formValues, EncounterTypeId.VAGINAL_EXAMINATION);
          formRef.value.resetForm();
          toastSuccess("Vaginal inspection saved successfully");
...
const _sfc_main$d = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    const onSubmit = async () => {
      try {
        const results = [
          breastExaminationsResultsRef.value?.onSubmit?.(),
          vaginalInspection2Ref.value?.onSubmit?.(),
          vaginalExaminationDoneRef.value?.onSubmit?.()
...
const MaternalExam = /* @__PURE__ */ _export_sfc(_sfc_main$d, [["__scopeId", "data-v-ca269720"]]);
...
const useFetalAssessmentStore = defineStore("fetalAssessmentStore", {
  state: () => ({
    // The total number of fetuses selected in the UI
    totalFetuses: 0,
    // Raw form values used for UI persistence: { 1: { "Fetal rate": "140" } }
    fetalData: {},
    // Structured OpenMRS observations ready for submission: { 1: [obs1, obs2] }
    validatedFetalObsData: {},
    // Patient ID can now explicitly be string or null
    current_patient_ID: null
  }),
  actions: {
    /**
     * Updates the raw form values for a specific fetus.
     */
    setFetusData(fetusNum, data) {
      if (!this.fetalData[fetusNum]) {
        this.fetalData[fetusNum] = {};
      }
      this.fetalData[fetusNum] = { ...this.fetalData[fetusNum], ...data };
    },
    /**
     * Sets the total count and prunes any data that exceeds the new count.
     */
    setTotalFetuses(count) {
      this.totalFetuses = count;
      Object.keys(this.fetalData).forEach((key) => {
        const fetusId = parseInt(key);
        if (fetusId > count) delete this.fetalData[fetusId];
      });
      Object.keys(this.validatedFetalObsData).forEach((key) => {
        const fetusId = parseInt(key);
        if (fetusId > count) delete this.validatedFetalObsData[fetusId];
      });
    },
    /**
     * Stores the built observations for a specific fetus.
     */
    setValidatedFetalData(fetusNum, obsArray) {
      this.validatedFetalObsData[fetusNum] = obsArray;
    },
    /**
     * Returns all validated observations flattened into a single array.
     */
    getBundledObs() {
      return Object.values(this.validatedFetalObsData).flat();
    },
    /**
     * Updates the patient ID. Accepts string or null.
     */
    ...
    /**
     * Fully clears the store after a successful encounter submission.
     */
    resetStore() {
      this.fetalData = {};
  ...
  setup(__props, { expose: __expose }) {
    ...
    const fetusDataForm = computed(() => {
      return [
        ...
        },
        {
          ...
          condition: (allFormValues) => allFormValues["Fetal heartbeat present?"] === "Yes",
          onChange: (value) => {
            if (value !== "Other") return { "specify the Fetal presentation": "" };
          }
        },
        {
    ...
    const caputreCurrentFetalAssessment = async () => {
      try {
        const errors = formRef.value?.validateForm();
        if (errors) {
          toastDanger(`Please correct the validation errors for Fetus #${props.fetus_number}.`);
          return null;
        }
        const formValues = formRef.value?.getFormValues() || {};
        formRef.value?.validateForm();
        if (formValues["Fetal heartbeat present?"] || formValues["Fetal rate"] || formValues["Repeated fetal rate"] || formValues["Fetal heart rate regularity"] || formValues["Select fetal presentation"] || formValues["specify the Fetal presentation"]) {
          const assessmentObs = await ObservationService.buildObsValues({
            ...formValues
            // "Fetus Identifier": `Fetus ${props.fetus_number}`
          });
          fetalStore.setValidatedFetalData(props.fetus_number, assessmentObs);
        }
      } catch (error) {
        console.error("Error capturing fetal assessment data", error);
...
const addFetus = /* @__PURE__ */ _export_sfc(_sfc_main$c, [["__scopeId", "data-v-47447378"]]);
...
const _sfc_main$a = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const hasData = (fetusNum) => {
      return !!fetalStore.fetalData[fetusNum] && Object.keys(fetalStore.fetalData[fetusNum]).length > 0;
    };
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock(Fragment, null, [
            ...
            (openBlock(true), createElementBlock(Fragment, null, renderList(numFetuses.value, (fetusNum) => {
              return openBlock(), createElementBlock("div", {
                ...
                createBaseVNode("div", _hoisted_6, [
                  ...
                  createBaseVNode("span", _hoisted_8, toDisplayString(hasData(fetusNum) ? "Complete" : "Pending"), 1)
                ]),
                createVNode(DynamicButton, {
                  name: hasData(fetusNum) ? "Edit" : "Add",
                  size: "small",
                  fill: "clear",
                  color: hasData(fetusNum) ? "success" : "primary",
                  "onClicked:btn": ($event) => unref(openFetalModal)(fetusNum),
                  class: "fetus-btn"
                }, null, 8, ["name", "color", "onClicked:btn"])
              ], 2);
            }), 128))
          ])) : createCommentVNode("", true)
        ])) : createCommentVNode("", true),
...
const FetusSelector = /* @__PURE__ */ _export_sfc(_sfc_main$a, [["__scopeId", "data-v-2b2d37b0"]]);
...
const _sfc_main$9 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    const isNumberKnown = ref(false);
    const handleFormUpdate = (allValues) => {
      isNumberKnown.value = allValues?.["Number of fetuses known"] === "Yes";
    };
    ...
    const onSubmit = async () => {
      ...
      const fetalAssessmentStore = useFetalAssessmentStore();
      const allFetuses = fetalAssessmentStore.validatedFetalObsData;
      const formValues = formRef.value.getFormValues() || {};
      if (formValues["Number of fetuses known"] || formValues["Symphysis-fundal height"]) {
        try {
          ...
          const knownFetusCountObsversation = {
            ...
            "value_numeric": fetalAssessmentStore.totalFetuses,
          ...
          const patient = await ObservationService.addObsToEncounterPatient(assessmentObs, EncounterTypeId.ASSESSMENT);
          await savePatientRecord(patient);
          fetalAssessmentStore.resetStore();
          formRef.value.resetForm();
        ...
        toastWarning("No Fetal Assessment data to save");
      }
    };
    __expose({
      onSubmit
