...
const usePastObstetricHistoryForm = () => {
  ...
  const pastPregnancyComplicationsOptions = [
    ...
    { value: "Perineal tear (3rd or 4th degree)", label: "Perineal tear (3rd or 4th degree)" },
  ...
  const neonatalComplicationsOptions = [
    { value: "None", label: "None", exclusive: true },
    { value: "Asphyxia", label: "Asphyxia" },
  ...
  const generateModeOfDeliveryFields = (numberOfChildren) => {
    ...
    for (let index2 = 0; index2 < numberOfChildren; index2++) {
      fields.push({
        componentType: "radioButtonField",
        header: `Specify mode of delivery (child ${index2 + 1})`,
        name: `Mode of delivery ${index2}`,
        required: true,
        options: modeOfDeliveryOptions,
        grid: { s: "12", md: "12" },
        validation: (value) => {
          if (!value || value === "") {
            return `Mode of delivery for child ${index2 + 1} is required`;
          }
          return null;
        }
      });
    ...
    for (let index2 = 0; index2 < numberOfChildren; index2++) {
      fields.push({
        ...
        header: `Reason for Caesarean section (child ${index2 + 1})`,
        name: `Reason for Caesarean section ${index2}`,
        type: "inline",
        icon: icons.editPen,
        required: true,
        grid: { s: "12", md: "6" },
        validation: (value, allFormValues) => {
          const modeId = allFormValues?.[`Mode of delivery ${index2}`];
          ...
          if (modeName === "Caesarean section" && (!value || value === "")) {
            return `Please specify reason for Caesarean section for child ${index2 + 1}`;
          }
          return null;
        },
    ...
    fields.push({
      ...
      condition: (allFormValues) => {
        ...
        return modeId.includes("Other") || modeId.includes("Caesarean section");
      }
    });
    for (let index2 = 0; index2 < numberOfChildren; index2++) {
      fields.push({
        componentType: "inputField",
        header: `Specify delivery details (child ${index2 + 1})`,
        name: `Specify ${index2}`,
        type: "inline",
        icon: icons.editPen,
        required: true,
        ...
        validation: (value, allFormValues) => {
          ...
          if ((modeName === "Other" || modeName === "Caesarean section") && (!value || value === "")) {
            return `Please specify details for child ${index2 + 1} since mode of delivery is ${modeName}`;
          }
          return null;
        },
        condition: (allFormValues) => {
  ...
  const pastNeonatalComplicationsOptionForm = computed(() => [
    {
      ...
      required: true,
      grid: { s: "12" },
      options: neonatalComplicationsOptions,
      validation: (value) => {
        if (!value || Array.isArray(value) && value.length === 0) {
...
const _sfc_main$8 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    const onSubmit = async () => {
      if (!obstetricFormRef.value || !pregnanciesComplicationFormRef.value || !neonatalComplicationFormRef.value) {
      ...
      const neonatalComplicationErrors = neonatalComplicationFormRef.value.validateForm();
      if (neonatalComplicationErrors) {
        toastWarning("Please fix validation errors in Past Neonatal Complications before submitting");
        return false;
      }
      if (validationErrors) {
...
const PastObstetricHistory = /* @__PURE__ */ _export_sfc(_sfc_main$8, [["__scopeId", "data-v-6fa932ce"]]);
  ...
  setup(__props, { expose: __expose }) {
    ...
    const onSubmit = async () => {
      ...
      try {
        ...
        await ObservationService.buildAddObsToEncounterPatient(surgeriesData, EncounterTypeId.Obstetrics_and_Gynecology);
        formKey.value++;
        surgeriesFormKey.value++;
        await nextTick();
...
const Gynaecology = /* @__PURE__ */ _export_sfc(_sfc_main$2, [["__scopeId", "data-v-566d512d"]]);
