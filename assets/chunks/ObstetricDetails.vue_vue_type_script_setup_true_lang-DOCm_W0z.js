import { c as computed, eI as create$3, eJ as create$5, eK as create$6, s as defineComponent, a3 as onMounted, n as nextTick, y as openBlock, O as createBlock, B as withCtx, C as createBaseVNode, A as createVNode, H as createCommentVNode, F as unref, b1 as personOutline, f as ref, K as modalController, w as watch, z as createElementBlock, bf as IonCardContent, a7 as IonLabel, a5 as createTextVNode, D as toDisplayString, J as Fragment, S as renderList, bN as IonCard, Q as normalizeClass, bd as IonCardHeader, bc as IonCardTitle, L as IonIcon, R as alertCircleOutline, bB as IonText, N as IonButton, e5 as createOutline, bJ as addOutline, x as resolveComponent } from './vendor-QOXikrLM.js';
import { bX as getAgeInDays, n as icons, P as PatientService, K as ObservationService, b as EncounterTypeId, z as StandardForm, F as DynamicButton, r as StandardModal, t as toastWarning, H as HisDate, G as toastSuccess, b7 as RelationshipService, S as Service, N as ConceptService, u as useDemographicsStore, C as useExposeFromStandardForm, J as savePatientRecord, _ as _export_sfc, a4 as getFieldValue } from '../index-Biv93APm.js';
import { F as FindRegisterPatient } from './FindRegisterPatient-DdjdBA2C.js';
import { l as lodashExports } from './lodash-CVfF-l1v.js';
import { u as useBabyDetails } from './useBabyDetails-Btn83KkR.js';
import { s as storeToRefs, d as defineStore } from './pinia-BRP2SQHv.js';
import { V as Validation } from './StandardValidations-CT1fmXJM.js';
...
const useDeliveryDetailsForm = (isFormLockedInput) => {
  ...
  const deliveryDetailsFormSection = computed(() => [
    ...
    },
    {
      ...
      options: [
        {
          label: "Skilled Health worker ( Nurse/Midwife/Clinician/Medical Doctor)",
          value: "Skilled Health worker ( Nurse/Midwife/Clinician/Medical Doctor)"
        },
        {
...
const DATE_OF_DELIVERY_CONCEPT_ID = 5599;
function readRawObsScalar(obs) {
  if (!obs) return null;
  const candidates = [obs.value_text, obs.value_datetime, obs.value_date, obs.value_numeric, obs.value];
  for (const c of candidates) {
    if (c != null && c !== "") return c;
  }
  return null;
}
function flattenObservationsDeep(encounterRows) {
  const out = [];
  const walk = (obs) => {
    if (!obs) return;
    out.push(obs);
    const kids = obs.groupMembers ?? obs.children ?? obs.child;
    if (Array.isArray(kids)) for (const k of kids) walk(k);
  };
  for (const e of encounterRows || []) {
    for (const o of e.obs || []) walk(o);
  }
  return out;
}
function latestObsPerConcept(allObs) {
  const latestObsMap = /* @__PURE__ */ new Map();
  allObs.forEach((obs) => {
    const name = obs.concept_name;
    const key = name != null && String(name).trim() !== "" ? String(name).trim() : `id:${obs.concept_id}`;
    const obsDatetime = new Date(obs.obs_datetime).getTime();
    if (!latestObsMap.has(key)) {
      latestObsMap.set(key, obs);
    } else {
      const existingObs = latestObsMap.get(key);
      const existingDatetime = new Date(existingObs.obs_datetime).getTime();
      if (obsDatetime > existingDatetime) latestObsMap.set(key, obs);
    }
  });
  return Array.from(latestObsMap.values());
}
async function getLatestLabourObsFlattened() {
  const rows = await ObservationService.getObsByEncounterId(EncounterTypeId.Labour_and_delivery_visit);
  return latestObsPerConcept(flattenObservationsDeep(rows));
}
async function getFirstObsByConcepts(obsArray, conceptNames, conceptIdFallback) {
  for (const name of conceptNames) {
    const obs = obsArray.find((o) => o.concept_name?.toLowerCase() === name.toLowerCase());
    if (!obs) continue;
    const v = readRawObsScalar(obs);
  ...
  if (conceptIdFallback != null) {
    const obs = obsArray.find((o) => o.concept_id === conceptIdFallback);
    if (obs) {
      const v = readRawObsScalar(obs);
      if (v != null && v !== "") return v;
      if (obs.value_coded != null && obs.value_coded !== "") {
...
async function extractObsValue(obsArray, conceptNames, conceptIdFallback) {
  const raw = await getFirstObsByConcepts(obsArray, conceptNames, conceptIdFallback);
...
const STAFF_OPTIONS = {
  skilled: "Skilled Health worker ( Nurse/Midwife/Clinician/Medical Doctor)",
  unskilled: "Unskilled (Patient attendant/ ward attendant)",
  tba: "Traditional birth attendant (TBA)"
};
function mapStaffFromLabour(raw) {
  const t = String(raw).toLowerCase().trim();
  if (t === STAFF_OPTIONS.skilled.toLowerCase()) return STAFF_OPTIONS.skilled;
  if (t === STAFF_OPTIONS.unskilled.toLowerCase()) return STAFF_OPTIONS.unskilled;
  if (t === STAFF_OPTIONS.tba.toLowerCase()) return STAFF_OPTIONS.tba;
  if (t.includes("traditional birth attendant") || t.includes("tba")) return STAFF_OPTIONS.tba;
  if (t.includes("unskilled") || t.includes("patient attendant") || t.includes("ward attendant") || t.includes("health surveillance assistant") || t.includes("hsa")) {
    return STAFF_OPTIONS.unskilled;
  }
  if (t.includes("skilled") || t.includes("nurse midwife") || t.includes("community midwife") || t.includes("midwife") || t.includes("cma") || t.includes("medical assistant") || t.includes("clinical technician") || t.includes("doctor")) {
    return STAFF_OPTIONS.skilled;
  }
  return null;
}
function mapStaffFromUserRoles() {
  const roles = Service.getUserRoles?.() ?? [];
  const roleStrings = roles.map((r) => typeof r === "string" ? r : r?.role ?? r?.name ?? "").map((s) => s.trim()).filter(Boolean);
  const t = roleStrings.join(" ").toLowerCase();
  if (t.includes("traditional birth attendant") || t.includes("tba")) return STAFF_OPTIONS.tba;
  if (t.includes("unskilled") || t.includes("patient attendant") || t.includes("ward attendant") || t.includes("hsa") || t.includes("health surveillance assistant")) {
    return STAFF_OPTIONS.unskilled;
  }
  if (t.includes("nurse") || t.includes("midwife") || t.includes("doctor") || t.includes("cma") || t.includes("medical assistant") || t.includes("clinical technician")) {
    return STAFF_OPTIONS.skilled;
  }
  return STAFF_OPTIONS.skilled;
}
...
function formatDateForFormInput(value) {
  if (value == null || value === "") return null;
  const formatted = HisDate.toStandardHisDisplayFormat(value);
  if (!formatted || /invalid/i.test(formatted)) return null;
  return formatted;
}
async function extractLabourPrefillValues(obsArray) {
  const [dateOfDelivery, timeOfDelivery, modeOfDelivery, placeOfDelivery, staffConductingDelivery, babyCondition, numberOfBabies] = await Promise.all([
    extractObsValue(obsArray, ["Date of delivery"], DATE_OF_DELIVERY_CONCEPT_ID),
    ...
    extractObsValue(obsArray, "Number of babies")
  ]);
  return {
    dateOfDelivery,
    timeOfDelivery,
...
function applyLabourPrefillToForm(formRef, values) {
  ...
  const dateDisplay = values.dateOfDelivery != null ? formatDateForFormInput(String(values.dateOfDelivery)) ?? String(values.dateOfDelivery).trim() : null;
  set("Date of delivery", dateDisplay);
  ...
  if (values.staffConductingDelivery != null) {
    const mapped = mapStaffFromLabour(String(values.staffConductingDelivery));
    if (mapped) formRef.setFormValue("Staff conducting delivery", mapped);
  }
...
async function runDeliveryDetailsPrefill(formRef) {
  ...
  const labourObs = await getLatestLabourObsFlattened();
  if (!labourObs?.length) {
    const current2 = formRef.getFormValues?.();
    if (!current2?.["Staff conducting delivery"]) {
      formRef.setFormValue("Staff conducting delivery", mapStaffFromUserRoles());
    }
    const deliveryFallback = await RelationshipService.getLatestDeliveryDate();
    if (deliveryFallback != null && deliveryFallback !== "") {
      const fmt = formatDateForFormInput(String(deliveryFallback));
      if (fmt) formRef.setFormValue("Date of delivery", fmt);
    }
    return result;
  }
  const values = await extractLabourPrefillValues(labourObs);
  Object.assign(result, applyLabourPrefillToForm(formRef, values));
  if (!formRef.getFormValues?.()?.["Date of delivery"]) {
    const deliveryFallback = await RelationshipService.getLatestDeliveryDate();
    if (deliveryFallback != null && deliveryFallback !== "") {
      const fmt = formatDateForFormInput(String(deliveryFallback));
      if (fmt) formRef.setFormValue("Date of delivery", fmt);
  ...
  const current = formRef.getFormValues?.();
  if (!current?.["Staff conducting delivery"]) {
    const mapped = values.staffConductingDelivery != null ? mapStaffFromLabour(String(values.staffConductingDelivery)) : null;
    formRef.setFormValue("Staff conducting delivery", mapped ?? mapStaffFromUserRoles());
  }
  return result;
}
...
const useHIVStatusAndTreatmentForm = () => {
  ...
  const hivStatusAndTreatmentFormSection = computed(() => {
    const isMotherUnknown = (allFormValues) => allFormValues["Mother HIV Status"] === "unknown" || allFormValues["Mother HIV Status"] === "negative";
  ...
  const obstetricDetailsFormSection = computed(() => {
    return [
      ...
      },
      {
        ...
        name: "Degree of Perineal tear",
        obsValueType: "value_coded",
        options: perinealTearOptions,
        grid: { s: "12" },
        condition: (allFormValues) => {
          return allFormValues["past pregnancy problems"]?.includes("Perineal tear");
        }
      },
      {
        componentType: "inputField",
