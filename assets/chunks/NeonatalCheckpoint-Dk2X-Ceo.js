import { s as defineComponent, aN as useRouter, f as ref, fF as onBeforeRouteLeave, a3 as onMounted, w as watch, y as openBlock, O as createBlock, B as withCtx, A as createVNode, F as unref, aI as IonContent, C as createBaseVNode, N as IonButton, L as IonIcon, G as closeCircleOutline, a5 as createTextVNode, fh as arrowForwardOutline, D as toDisplayString, bx as IonPage, c as computed } from './vendor-8vOj_QAN.js';
import { s as storeToRefs } from './pinia-Drf8jDfn.js';
import { u as useDemographicsStore, b as EncounterTypeId, T as Toolbar, t as toastWarning, S as Service, E as EncounterService, k as alertConfirmation, _ as _export_sfc } from '../index-Bn2sysQf.js';
import { g as getSequentiallyCompletedStepCount } from './clinicalReviewWorkflow-BPsfuTGc.js';
import { C as ClinicalReviewProgressBar } from './ClinicalReviewProgressBar-DBQuV8v_.js';

const _hoisted_1 = { class: "checkpoint-container" };
const _hoisted_2 = { class: "control-buttons" };
const _sfc_main = /* @__PURE__ */ defineComponent({
  __name: "NeonatalCheckpoint",
  setup(__props) {
    const router = useRouter();
    const demographicsStore = useDemographicsStore();
    const { patient } = storeToRefs(demographicsStore);
    const loading = ref(false);
    const savedEncounters = ref([]);
    const isTriageCompleted = ref(false);
    const allowCheckpointNavigation = ref(false);
    const workflowSteps = ref([
      {
        encounterType: EncounterTypeId.NEONATAL_SIGNS_SYMPTOMS,
        title: "Signs & Symptoms",
        route: "/neonatal/signs-symptoms",
        encounterNames: ["NEONATAL SIGNS & SYMPTOMS", "NEONATAL SIGNS SYMPTOMS"],
        isCompleted: false
      },
      {
        encounterType: EncounterTypeId.NEONATAL_REVIEW_OF_SYSTEMS,
        title: "Review of Systems",
        route: "/neonatal/review-of-systems",
        encounterNames: ["NEONATAL REVIEW OF SYSTEMS"],
        isCompleted: false
      },
      {
        encounterType: EncounterTypeId.PHYSICAL_EXAMINATION_BABY,
        title: "General Examination",
        route: "/neonatal/general-examination",
        encounterNames: ["PHYSICAL EXAMINATION BABY", "NEONATAL GENERAL EXAMINATION"],
        isCompleted: false
      },
      {
        encounterType: EncounterTypeId.VITALS,
        title: "Vitals",
        route: "/neonatal/vitals",
        encounterNames: ["VITALS", "NEONATAL VITALS"],
        isCompleted: false
      },
      {
        encounterType: EncounterTypeId.NEONATAL_SYSTEMIC_EXAMINATION,
        title: "Systemic Examination",
        route: "/neonatal/systemic-examination",
        encounterNames: ["NEONATAL SYSTEMIC EXAMINATION"],
        isCompleted: false
      },
      {
        encounterType: EncounterTypeId.NEONATAL_ENROLLMENT,
        title: "Profiling",
        route: "/neonatal/enrollment",
        encounterNames: ["NEONATAL ENROLLMENT", "NEONATAL ENROLMENT", "NEONATAL_ENROLLMENT", "NEONATAL_ENROLMENT", "ENROLLMENT", "ENROLMENT"],
        isCompleted: false
      },
      {
        encounterType: EncounterTypeId.DIAGNOSIS,
        title: "Diagnosis",
        route: "/neonatal/diagnosis",
        encounterNames: ["DIAGNOSIS", "NEONATAL DIAGNOSIS"],
        isCompleted: false
      },
      {
        encounterType: EncounterTypeId.LAB_ORDERS,
        title: "Investigations",
        route: "/neonatal/investigation",
        encounterNames: ["LAB ORDERS", "LAB RESULTS"],
        isCompleted: false
      },
      {
        encounterType: EncounterTypeId.TREATMENT,
        title: "Treatment Plan",
        route: "/neonatal/treatment-plan",
        encounterNames: ["TREATMENT", "NEONATAL TREATMENT PLAN"],
        isCompleted: false
      },
      {
        encounterType: EncounterTypeId.NEONATAL_CLINICAL_REVIEW_OUTCOMES,
        title: "Clinical Review Outcomes",
        route: "/neonatal/clinical-review-outcomes",
        encounterNames: ["NEONATAL ADMISSION OUTCOMES", "ADMISSION OUTCOMES", "NEONATAL CLINICAL REVIEW OUTCOMES", "CLINICAL REVIEW OUTCOMES"],
        isCompleted: false
      },
      {
        title: "Clinical Review Sign Off",
        route: "/neonatal/clinical-review-signoff",
        encounterNames: ["NEONATAL SIGNOFF"],
        isCompleted: false
      }
    ]);
    const normalizeEncounterName = (name) => (name ?? "").trim().toUpperCase();
    const nextButtonText = computed(() => {
      if (!isTriageCompleted.value) {
        return "Next: Triage";
      }
      const nextStep = workflowSteps.value.find((step) => !step.isCompleted);
      if (nextStep) {
        return `Next: ${nextStep.title}`;
      }
      return "Print Summary";
    });
    const allCompleted = computed(() => workflowSteps.value.every((step) => step.isCompleted));
    const getEncounterTypeNameById = (encounterTypeId) => {
      const encounterTypeMap = {
        [EncounterTypeId.NEONATAL_SIGNS_SYMPTOMS]: "NEONATAL SIGNS & SYMPTOMS",
        [EncounterTypeId.NEONATAL_REVIEW_OF_SYSTEMS]: "NEONATAL REVIEW OF SYSTEMS",
        [EncounterTypeId.PHYSICAL_EXAMINATION_BABY]: "PHYSICAL EXAMINATION BABY",
        [EncounterTypeId.VITALS]: "VITALS",
        [EncounterTypeId.NEONATAL_SYSTEMIC_EXAMINATION]: "NEONATAL SYSTEMIC EXAMINATION",
        [EncounterTypeId.NEONATAL_ENROLLMENT]: "NEONATAL ENROLLMENT",
        [EncounterTypeId.DIAGNOSIS]: "DIAGNOSIS",
        [EncounterTypeId.LAB_ORDERS]: "LAB ORDERS",
        [EncounterTypeId.TREATMENT]: "TREATMENT",
        [EncounterTypeId.NEONATAL_CLINICAL_REVIEW_OUTCOMES]: "NEONATAL CLINICAL REVIEW OUTCOMES",
        [EncounterTypeId.NEONATAL_TRIAGE]: "NEONATAL TRIAGE",
        [EncounterTypeId.NEONATAL_EMERGENCY_TRIAGE]: "NEONATAL EMERGENCY TRIAGE"
      };
      return encounterTypeMap[encounterTypeId] || null;
    };
    const getEncounterNamesFromDemographicsStore = () => {
      if (!patient.value) {
        return [];
      }
      const encounterTypeIds = /* @__PURE__ */ new Set();
      if (Array.isArray(patient.value.observations)) {
        patient.value.observations.forEach((obs) => {
          const encounterTypeId = obs?.encounter_type;
          if (typeof encounterTypeId === "number") {
            encounterTypeIds.add(encounterTypeId);
          }
        });
      }
      const encounterKeys = [
        "neonatal_signs_symptoms",
        "neonatal_review_of_systems",
        "neonatal_physical_examination_baby",
        "neonatal_vitals",
        "neonatal_systemic_examination",
        "neonatal_enrollments",
        "neonatal_diagnosis",
        "neonatal_lab_orders",
        "neonatal_treatment",
        "neonatal_clinical_review_outcomes",
        "neonatal_triage",
        "neonatal_emergency_triage"
      ];
      encounterKeys.forEach((key) => {
        const encounterData = patient.value[key];
        if (encounterData) {
          if (Array.isArray(encounterData.saved)) {
            encounterData.saved.forEach((enc) => {
              const encTypeId = enc?.encounter_type_id;
              if (typeof encTypeId === "number") {
                encounterTypeIds.add(encTypeId);
              }
            });
          }
          if (Array.isArray(encounterData.unsaved)) {
            encounterData.unsaved.forEach((enc) => {
              const encTypeId = enc?.encounter_type_id;
              if (typeof encTypeId === "number") {
                encounterTypeIds.add(encTypeId);
              }
            });
          }
        }
      });
      const encounterNames = [];
      encounterTypeIds.forEach((id) => {
        const name = getEncounterTypeNameById(id);
        if (name) {
          encounterNames.push(name);
        }
      });
      return encounterNames;
    };
    const hasLabOrders = () => {
      if (!patient.value?.labOrders) return false;
      const { saved = [], unsaved = [] } = patient.value.labOrders;
      return saved.length > 0 || unsaved.length > 0;
    };
    const loadProgress = async () => {
      if (!patient.value?.patientID) {
        toastWarning("No patient selected");
        router.push("/patient-profile");
        return;
      }
      loading.value = true;
      try {
        const encounterNamesFromStore = getEncounterNamesFromDemographicsStore();
        let generalEncounterNames = [];
        const isOffline = Service.getPouchDbStatus() || Service.getLanConnectionStatus();
        try {
          const allEncounters = await EncounterService.getEncounters(patient.value.patientID);
          const sessionDate = Service.getSessionDate();
          generalEncounterNames = Array.isArray(allEncounters) ? allEncounters.filter((e) => {
            const name = normalizeEncounterName(e.type?.name || e.encounter_type_name);
            if (name === "LAB ORDERS" || name === "LAB RESULTS") {
              if (isOffline) {
                return true;
              }
              const encounterDate = e.encounter_datetime?.split("T")[0];
              return encounterDate === sessionDate;
            }
            return true;
          }).map((e) => e.type?.name || e.encounter_type_name).filter(Boolean) : [];
        } catch (e) {
          console.warn("[NeonatalCheckpoint] Failed to fetch general encounters", e);
        }
        const combinedEncounters = [.../* @__PURE__ */ new Set([...encounterNamesFromStore, ...generalEncounterNames])];
        if (hasLabOrders()) {
          combinedEncounters.push("LAB ORDERS");
        }
        savedEncounters.value = combinedEncounters;
        const normalizedEncounterSet = new Set(combinedEncounters.map((encounter) => normalizeEncounterName(encounter)));
        const hasEncounter = (names) => names.some((name) => normalizedEncounterSet.has(normalizeEncounterName(name)));
        isTriageCompleted.value = hasEncounter([
          "NEONATAL TRIAGE",
          "NEONATAL EMERGENCY TRIAGE",
          "EMERGENCY TRIAGE",
          "TRIAGE",
          "NEONATAL_TRIAGE",
          "NEONATAL_EMERGENCY_TRIAGE"
        ]);
        const completedCount = getSequentiallyCompletedStepCount(combinedEncounters, workflowSteps.value);
        workflowSteps.value.forEach((step, index) => {
          step.isCompleted = index < completedCount;
        });
      } catch (error) {
        console.error("Error loading progress:", error);
        toastWarning("Unable to load progress");
      } finally {
        loading.value = false;
      }
    };
    const navigateAway = (route, query) => {
      allowCheckpointNavigation.value = true;
      if (query && Object.keys(query).length) {
        router.push({ path: route, query });
      } else {
        router.push(route);
      }
    };
    const handleNext = () => {
      if (!isTriageCompleted.value) {
        navigateAway("/neonatal/triage");
        return;
      }
      const nextStep = workflowSteps.value.find((step) => !step.isCompleted);
      if (nextStep) {
        navigateAway(nextStep.route, nextStep.query);
      } else if (allCompleted.value) {
        navigateAway("/neonatal/clinical-review-outcomes");
      }
    };
    const handleCancel = async () => {
      const confirmed = await alertConfirmation(
        "Are you sure you want to exit the neonatal clinical review workflow and return to the patient profile?",
        {
          header: "Exit clinical review workflow",
          confirmBtnLabel: "Exit"}
      );
      if (confirmed) {
        navigateAway("/patient-profile");
      }
    };
    onBeforeRouteLeave((to, from, next) => {
      if (allowCheckpointNavigation.value) {
        allowCheckpointNavigation.value = false;
        next();
        return;
      }
      toastWarning("Use the Cancel button if you need to exit the checkpoint.");
      next(false);
    });
    onMounted(() => {
      loadProgress();
    });
    watch(
      () => patient.value?.patientID,
      (newId) => {
        if (newId) {
          loadProgress();
        }
      }
    );
    watch(
      () => router.currentRoute.value.fullPath,
      (currentPath) => {
        if (currentPath === "/neonatal/checkpoint") {
          loadProgress();
        }
      }
    );
    return (_ctx, _cache) => {
      return openBlock(), createBlock(unref(IonPage), null, {
        default: withCtx(() => [
          createVNode(Toolbar),
          createVNode(unref(IonContent), { class: "ion-padding" }, {
            default: withCtx(() => [
              createBaseVNode("div", _hoisted_1, [
                createVNode(ClinicalReviewProgressBar),
                createBaseVNode("div", _hoisted_2, [
                  createVNode(unref(IonButton), {
                    fill: "solid",
                    color: "danger",
                    class: "cancel-button",
                    onClick: handleCancel
                  }, {
                    default: withCtx(() => [
                      createVNode(unref(IonIcon), {
                        slot: "start",
                        icon: unref(closeCircleOutline)
                      }, null, 8, ["icon"]),
                      _cache[0] || (_cache[0] = createTextVNode(" Cancel ", -1))
                    ]),
                    _: 1
                  }),
                  createVNode(unref(IonButton), {
                    fill: "solid",
                    color: "primary",
                    onClick: handleNext,
                    disabled: loading.value
                  }, {
                    default: withCtx(() => [
                      createVNode(unref(IonIcon), {
                        slot: "end",
                        icon: unref(arrowForwardOutline)
                      }, null, 8, ["icon"]),
                      createTextVNode(" " + toDisplayString(nextButtonText.value), 1)
                    ]),
                    _: 1
                  }, 8, ["disabled"])
                ])
              ])
            ]),
            _: 1
          })
        ]),
        _: 1
      });
    };
  }
});

const NeonatalCheckpoint = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-c9b08c5c"]]);

export { NeonatalCheckpoint as default };
