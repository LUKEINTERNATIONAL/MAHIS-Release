import { s as defineComponent, w as watch, y as openBlock, z as createElementBlock, A as createVNode, F as unref, f as ref, c as computed, aH as IonContent, bw as IonPage, aM as useRouter, f1 as provide, x as resolveComponent, O as createBlock, B as withCtx } from './vendor-XNfOCpdb.js';
import { s as storeToRefs } from './pinia-CtfxbWof.js';
import { z as StandardForm, _ as _export_sfc, u as useDemographicsStore, T as Toolbar, t as toastWarning, G as toastSuccess, x as toastDanger } from '../index-DjgSdAC3.js';
import { D as DemographicBar } from './DemographicBar-C6xRBdRR.js';
import { u as useNeonatalExamStore, N as NeonatalStepper } from './NeonatalStepper-CCDDeUs3.js';
import { n as neonatalGeneralExaminationSections, a as neonatalGeneralExaminationFormKey } from './neonatal_service-BvbwyxIB.js';
...
const _sfc_main$3 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    const withGenderDisabled = (options) => {
      if (!Array.isArray(options)) return options;
      return options.map((opt) => {
        const label = String(opt?.label || "");
        const isMaleOpt = /male/i.test(label);
        const isFemaleOpt = /female/i.test(label);
        let disabled = false;
        if (gender.value === "M") {
          disabled = isFemaleOpt;
        } else if (gender.value === "F") {
          disabled = isMaleOpt;
        } else {
          disabled = isMaleOpt || isFemaleOpt;
        }
        return { ...opt, disabled };
      });
    };
    const formData = computed(() => {
      return baseFormData.map((field) => {
        if (field.name === "genitaliaAssessment" && Array.isArray(field.options)) {
          return {
            ...field,
            options: withGenderDisabled(field.options)
          };
        }
        return field;
      });
...
const GenitalAndAnusAssessmentForm = /* @__PURE__ */ _export_sfc(_sfc_main$3, [["__scopeId", "data-v-f5c14765"]]);
