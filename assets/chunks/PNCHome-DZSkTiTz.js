import { c as computed, s as defineComponent, y as openBlock, O as createBlock, B as withCtx, C as createBaseVNode, A as createVNode, F as unref, v as documentTextOutline, K as modalController, aN as useRouter, w as watch, a3 as onMounted, aI as IonContent, b$ as chevronBackOutline, av as b, e$ as paperPlaneOutline, dt as calendar, f0 as checkmarkDoneOutline, z as createElementBlock, J as Fragment, S as renderList, bx as IonPage, f as ref, cg as personCircle, aP as people, ck as medkit } from './vendor-QOXikrLM.js';
import { n as icons, z as StandardForm, F as DynamicButton, r as StandardModal, C as useExposeFromStandardForm, t as toastWarning, K as ObservationService, b as EncounterTypeId, G as toastSuccess, u as useDemographicsStore, T as Toolbar, o as createModal, _ as _export_sfc } from '../index-SKNIAGXC.js';
import { D as DemographicBar } from './DemographicBar-BK2rzQql.js';
import { D as DashboardCards } from './DashboardCards-DuV-DTL5.js';
import { s as storeToRefs } from './pinia-BRP2SQHv.js';
import { u as useLocation } from './useLocation-tnK13J5E.js';
import { N as NextAppointmentModal } from './NextAppointment-Dcmncqhr.js';
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const postNCardsData = [{ title: "Postnatal visit details", path: "/pnc/visit", icon: medkit, isSaved: false }];
    ...
    const syncToggleFromEncounters = async () => {
      try {
        const [latestMotherObs, latestBabyObs] = await Promise.all([
          ObservationService.getLatestObsByEncounterId(EncounterTypeId.PNC_WARD_MONITORING_MOTHER),
          ObservationService.getLatestObsByEncounterId(EncounterTypeId.PNC_WARD_MONITORING_BABY)
        ]);
        const hasMother = Array.isArray(latestMotherObs) && latestMotherObs.length > 0;
        const hasBaby = Array.isArray(latestBabyObs) && latestBabyObs.length > 0;
        if (hasMother && hasBaby) pncHomeToggle.value = true;
      } catch (error) {
        console.error("Error syncing PNC toggle from encounters:", error);
      }
    };
    watch(
      () => patient.value?.patientID,
      async () => {
        pncHomeToggle.value = false;
        await syncToggleFromEncounters();
      },
      { immediate: true }
    );
    watch(
      () => patient.value?.observations?.length,
      async () => {
        await syncToggleFromEncounters();
      }
    );
...
const PNCHome = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-4c9395a9"]]);
