...
import { z as StandardForm, _ as _export_sfc, u as useDemographicsStore, E as EncounterService, S as Service, x as toastDanger, T as Toolbar, t as toastWarning, G as toastSuccess } from '../index-DcHp0H-N.js';
import { j as neonatalReviewOfSystemsSections, b as neonatalReviewOfSystemsFormKey, N as NeonatalStepper, H as useReviewOfSystemsStore } from './NeonatalStepper-BE3on7ue.js';
...
const _sfc_main$4 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    const deriveWeightTrendFromPercentage = (percentage) => {
      ...
      if (percentage >= 10) return "weight_loss_above_10";
      if (percentage > 0) return "weight_loss_below_10";
    ...
    const syncWeightTrendFromPercentage = (percentage) => {
      ...
      if (!calculatedTrend) return;
      if (rosForm.weight_trend_since_birth === calculatedTrend) return;
...
const GrowthSection = /* @__PURE__ */ _export_sfc(_sfc_main$4, [["__scopeId", "data-v-80a5fa1f"]]);
  ...
  async loadPatientClinicalData() {
    try {
      ...
      if (accumulatedData.BirthWeight > 0 && accumulatedData.Weight > 0) {
        const rawWeightLossPercentage = (accumulatedData.BirthWeight - accumulatedData.Weight) * 100 / accumulatedData.BirthWeight;
        if (rawWeightLossPercentage > 0) {
          const roundedPercentage = Number(rawWeightLossPercentage.toFixed(1));
          accumulatedData.weight_loss_percentage = roundedPercentage;
          ...
          accumulatedData.weight_loss_percentage = 0;
    ...
    const prefillWeightTrend = async () => {
      try {
        ...
        if (patientId) {
          ...
          const { weight_loss_category: calculatedValue, weight_loss_percentage: calculatedPercentage } = weightLossCategory;
          if (calculatedValue) {
            reviewOfSystemsFormData.weight_trend_since_birth = calculatedValue;
          }
          reviewOfSystemsFormData.weight_loss_percentage = calculatedPercentage ?? null;
        }
      } catch (error) {
...
const ReviewOfSystems = /* @__PURE__ */ _export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-075e6a03"]]);
