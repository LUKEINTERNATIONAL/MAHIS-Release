import { s as defineComponent, h as inject, w as watch, y as openBlock, z as createElementBlock, A as createVNode, F as unref, f as ref, bx as IonPage, aI as IonContent, aN as useRouter, f3 as provide, x as resolveComponent, O as createBlock, B as withCtx } from './vendor-QOXikrLM.js';
import { s as storeToRefs } from './pinia-BRP2SQHv.js';
import { z as StandardForm, _ as _export_sfc, T as Toolbar, u as useDemographicsStore, t as toastWarning, G as toastSuccess, x as toastDanger } from '../index-SKNIAGXC.js';
import { g as neonatalReviewOfSystemsSections, n as neonatalReviewOfSystemsFormKey, N as NeonatalStepper, L as useReviewOfSystemsStore } from './NeonatalStepper-DJUdVdam.js';
...
const FeedingHistorySection = /* @__PURE__ */ _export_sfc(_sfc_main$7, [["__scopeId", "data-v-a25a2bb1"]]);
...
const _sfc_main$6 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    const syncFormValues = (values) => {
      ...
      rosForm.has_baby_passed_stool = values.has_baby_passed_stool || "";
      ...
      rosForm.other_color_of_stools = values.other_color_of_stools || "";
...
const BowelMovementsSection = /* @__PURE__ */ _export_sfc(_sfc_main$6, [["__scopeId", "data-v-625fe050"]]);
...
const _sfc_main$5 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    const syncFormValues = (values) => {
      ...
      rosForm.is_baby_catheterized = values.is_baby_catheterized || "";
...
const UrineOutputSection = /* @__PURE__ */ _export_sfc(_sfc_main$5, [["__scopeId", "data-v-b57df0fe"]]);
...
const _sfc_main$4 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    const syncFormValues = (values) => {
      if (!rosForm) return;
      if (values.weight_trend_since_birth) {
        rosForm.weight_trend_since_birth = values.weight_trend_since_birth;
      }
    };
    let latestWeightLossPercentage = null;
    const applyWeightLossPercentageToField = (value) => {
      latestWeightLossPercentage = value;
      if (!formRef.value) {
        return;
      }
      const normalizedValue = value === null || value === void 0 ? "" : value;
      formRef.value.setFormValue("weight_loss_percentage", normalizedValue);
    };
    const deriveWeightTrendFromPercentage = (percentage) => {
      if (percentage == null) {
        return "weight_loss_none";
      } else if (percentage >= 10) {
        return "weight_loss_above_10";
      } else if (percentage > 0) {
        return "weight_loss_below_10";
      } else {
        return "weight_loss_none";
      }
    };
    const syncWeightTrendFromPercentage = (percentage) => {
      if (!rosForm) {
        return;
      }
      const calculatedTrend = deriveWeightTrendFromPercentage(percentage);
      if (!calculatedTrend) {
        return;
      }
      if (rosForm.weight_trend_since_birth === calculatedTrend) {
        return;
      }
      rosForm.weight_trend_since_birth = calculatedTrend;
      if (formRef.value) {
        formRef.value.setFormValue("weight_trend_since_birth", calculatedTrend);
      }
    };
    watch(
      () => formRef.value?.formValues,
    ...
    watch(
      ...
      (instance) => {
        if (instance?.getFormValues) {
          const formValues = instance.getFormValues();
          syncFormValues(formValues);
        }
        applyWeightLossPercentageToField(latestWeightLossPercentage ?? null);
      },
      { immediate: true }
    );
    watch(
      () => rosForm?.weight_loss_percentage,
      (newValue) => {
        const formatted = newValue ?? null;
        applyWeightLossPercentageToField(formatted);
        syncWeightTrendFromPercentage(formatted);
      },
      { immediate: true }
    );
    watch(
    ...
    watch(
      () => formRef.value,
      (newFormRef, oldFormRef) => {
        if (newFormRef && !oldFormRef && rosForm?.weight_trend_since_birth) {
          newFormRef.setFormValue("weight_trend_since_birth", rosForm.weight_trend_since_birth);
        }
      },
      { immediate: false }
    );
...
const GrowthSection = /* @__PURE__ */ _export_sfc(_sfc_main$4, [["__scopeId", "data-v-0de3b3ff"]]);
...
const MouthOropharynxSection = /* @__PURE__ */ _export_sfc(_sfc_main$3, [["__scopeId", "data-v-fc5c83d3"]]);
...
const AbdomenSection = /* @__PURE__ */ _export_sfc(_sfc_main$2, [["__scopeId", "data-v-f83dc14b"]]);
...
const MusculoskeletalSection = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["__scopeId", "data-v-fcac21c8"]]);
...
const _sfc_main = defineComponent({
  ...
  setup() {
    ...
    const syncForPatient = async () => {
      const { prefillOk, error } = await reviewOfSystemsStore.syncForRawPatientId(patient.value?.patientID);
      if (!prefillOk) {
        console.error("Failed to prefill weight trend:", error);
      }
    };
    router.isReady().then(async () => {
      await syncForPatient();
    });
    watch(
    ...
    const getSaveFunction = (currentIndex) => {
      if (currentIndex === stepperData.length - 1) {
        return async () => {
          ...
          try {
            const saveResult = await reviewOfSystemsStore.saveAssessment(patientId);
            if (!saveResult.ok) {
...
const ReviewOfSystems = /* @__PURE__ */ _export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-b59f17cb"]]);
