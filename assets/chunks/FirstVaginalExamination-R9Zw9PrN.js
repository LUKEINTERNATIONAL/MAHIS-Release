import { c as computed, s as defineComponent, y as openBlock, z as createElementBlock, A as createVNode, B as withCtx, F as unref, bf as IonCardContent, C as createBaseVNode, bN as IonCard } from './vendor-8vOj_QAN.js';
import { n as icons, u as useDemographicsStore, z as StandardForm, C as useExposeFromStandardForm, K as ObservationService, b as EncounterTypeId, G as toastSuccess, _ as _export_sfc, y as StandardValidations } from '../index-Bn2sysQf.js';
import { s as storeToRefs } from './pinia-Drf8jDfn.js';

const HAEMOGLOBIN_FIELD = "Haemoglobin level";
const FETAL_HEART_RATE_FIELD = "Fetal Heart Rate";
const parseHaemoglobinGPerDl = (raw) => {
  if (raw === "" || raw === null || raw === void 0) return null;
  const n = Number(raw);
  return Number.isFinite(n) ? n : null;
};
const parseFetalHeartRateBpm = (raw) => {
  if (raw === "" || raw === null || raw === void 0) return null;
  const n = Number(raw);
  return Number.isFinite(n) ? n : null;
};
    ...
    return [
      ...
      },
      {
        ...
        condition: async (formValues) => {
          const hb = parseHaemoglobinGPerDl(formValues[HAEMOGLOBIN_FIELD]);
          if (hb === null) return false;
          if (hb < 8) {
            return {
              backgroundColor: "#FEE4E2",
              icon: icons.dangerAlert,
              textColor: "#B42318",
              value: "Severe anaemia: haemoglobin is below 8 g/dL. Urgent clinical review is recommended.",
              name: "Haemoglobin severe anaemia alert"
            };
          }
          if (hb < 12) {
            return {
              ...
              textColor: "#B54708",
              value: "Moderate anaemia: haemoglobin is from 8 g/dL up to below 12 g/dL. Follow local anaemia management protocol.",
              name: "Haemoglobin moderate anaemia alert"
            };
          }
          if (hb >= 12 && hb <= 14) {
            return {
              backgroundColor: "#D1FADF",
              icon: icons.successAlert,
              textColor: "#027A48",
              value: "Normal: haemoglobin is within the expected range (12–14 g/dL).",
              name: "Haemoglobin normal range alert"
            };
          }
          if (hb >= 15) {
            return {
              ...
              icon: icons.warningAlert,
              textColor: "#B54708",
              value: "Abnormal: haemoglobin is 15 g/dL or above. Verify the result and assess as clinically appropriate.",
              name: "Haemoglobin elevated alert"
            };
          }
          return false;
        }
      ...
      },
      {
        ...
        validation: (value) => {
          ...
          if (numericValue < 12 || numericValue > 50) return "Fundal Height must be between 12 and 42 cm";
          return null;
        }
      },
      {
      ...
      },
      {
        ...
        validation: (value) => {
          ...
          if (numericValue < 110 || numericValue > 160) {
            return "Fetal Heart Rate must be between 110 and 160 bpm";
          }
          return null;
        }
      },
      {
        ...
        grid: { s: "12" },
        condition: async (formValues) => {
          const fhr = parseFetalHeartRateBpm(formValues[FETAL_HEART_RATE_FIELD]);
          if (fhr === null) return false;
          if (fhr < 110) {
            return {
              backgroundColor: "#FEE4E2",
              icon: icons.dangerAlert,
              textColor: "#B42318",
              value: "Fetal bradycardia: fetal heart rate is below 110 bpm. Assess urgently per protocol.",
              name: "Fetal heart rate bradycardia alert"
            };
          }
          if (fhr > 160) {
            return {
              ...
              icon: icons.warningAlert,
              textColor: "#B54708",
              value: "Fetal tachycardia: fetal heart rate is above 160 bpm. Assess per protocol.",
              name: "Fetal heart rate tachycardia alert"
            };
          }
          return false;
        }
...
const PhysicalExamination = /* @__PURE__ */ _export_sfc(_sfc_main$2, [["__scopeId", "data-v-a6c54916"]]);
...
const useFirstVaginalExaminationForm = () => {
  const firstVaginalExaminationFormSection = computed(() => {
    return [
      {
        componentType: "radioButtonField",
        header: "Vulva Inspection",
        name: "Vulva Inspection",
        type: "inline",
        obsValueType: "value_coded",
        options: [
          {
...
const FirstVaginalExamination = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-5fb062ae"]]);
