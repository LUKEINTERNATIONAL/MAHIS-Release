import { f as ref, s as defineComponent, w as watch, y as openBlock, z as createElementBlock, J as Fragment, S as renderList, C as createBaseVNode, D as toDisplayString, F as unref, Q as normalizeClass, A as createVNode, B as withCtx, L as IonIcon, c6 as checkmarkCircleOutline, N as IonButton, dk as eyeOutline, aU as medicalOutline, H as createCommentVNode, c as computed, bX as trashOutline, O as createBlock, af as IonRow, aB as IonCol, K as modalController, aN as useRouter, bO as useRoute, cm as onBeforeUnmount, a3 as onMounted, aI as IonContent, R as alertCircleOutline, bN as IonCard, ck as medkit, a5 as createTextVNode, a6 as IonInput, bh as IonFooter, bx as IonPage } from './vendor-QOXikrLM.js';
import { s as storeToRefs } from './pinia-BRP2SQHv.js';
import { E as EIRreportsStore } from './EIRreportsStore-C7JjsYYW.js';
import { N as NavigationMenu } from './NavigationMenu-Btt-dh2J.js';
import { V as ViewToggleComponent } from './ViewToggleComponent-Di-J_aG2.js';
import { N as ConceptService, b as EncounterTypeId, a as useProgramStore, H as HisDate, _ as _export_sfc, aZ as AppEncounterService, P as PatientService, S as Service, u as useDemographicsStore, J as savePatientRecord, G as toastSuccess, bY as toastDanger, bI as DrugOrderService, y as StandardValidations, z as StandardForm, F as DynamicButton, r as StandardModal, C as useExposeFromStandardForm, k as alertConfirmation, t as toastWarning, e as useGeneralStore, q as usePatientList, o as createModal, aN as PrintoutService, v as closeVisit, x as toastDanger$1, bZ as checkLiveAPIStatus, g as getPouchDBRecords } from '../index-Biv93APm.js';
import { a as DRUG_FREQUENCIES } from './drug_prescription_service-YG5QAC-_.js';
    ...
    for (const group of observations) {
      ...
      for (const obs of group.obs) {
        if (obs.obs_datetime) {
          ...
          const programId = obs.program_id;
          const key = `${date}_${programId}`;
          ...
          grouped[key].push({
            ...
            program: programMap.get(programId) || "N/A",
            ...obs
          });
        }
      }
...
const _hoisted_1$2 = { class: "diagnosis-container" };
...
const DiagnosisSummary = /* @__PURE__ */ _export_sfc(_sfc_main$2, [["__scopeId", "data-v-ebea2b59"]]);
...
class DispensationService extends AppEncounterService {
  ...
  async voidOrderOffline(medication, reason = "Out of stock") {
    const demographicsStore = useDemographicsStore();
    const { patient } = storeToRefs(demographicsStore);
    const patientData = JSON.parse(JSON.stringify(patient.value));
    const voidObject = {
    ...
    (patientData.voidedDrugOders ??= {}).unsaved ??= [];
    patientData.voidedDrugOders.unsaved.push({
      provider_id: this.providerID,
      program_id: medication.program_id,
      patient_id: this.patientID,
      voidedDrugOders: [voidObject]
    });
    await savePatientRecord(patientData);
    toastSuccess("Order marked as out of stock offline");
  }
}

function useVoidDrugOrder(medication) {
  console.log("Voiding medication:", medication);
  const demographicsStore = useDemographicsStore();
  const { patient } = storeToRefs(demographicsStore);
  const voidOrderFormData = computed(() => {
    return [
      {
        componentType: "radioButtonField",
        name: "reason",
        header: `Reason for Voiding: ${medication?.drug_name}`,
        options: [
          { label: "Out of Stock", value: "Out of Stock" },
          { label: "Incorrect Dose", value: "Incorrect Dose" },
          { label: "Patient Refused", value: "Patient Refused" },
          { label: "Clinical Error", value: "Clinical Error" },
  ...
  const processVoiding = async (formData) => {
    const dispensationService = new DispensationService();
    const finalReason = formData.reason === "Other" ? `Other: ${formData.other_reason || "No details"}` : formData.reason;
    const voidEntry = {
      provider_id: dispensationService.providerID,
      program_id: medication?.program_id,
      patient_id: dispensationService.patientID,
      voidedDrugOders: [
        {
          drug_order_id: medication?.order_id,
          date: dispensationService.date,
          reason: finalReason
        }
      ]
    };
    const patientData = JSON.parse(JSON.stringify(patient.value));
    (patientData.voidedDrugOders ??= {}).unsaved ??= [];
    patientData.voidedDrugOders.unsaved.push(voidEntry);
    const PatientData = await savePatientRecord(patientData);
...
const _hoisted_1$1 = { class: "positionCenter" };
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  __name: "VoidDrugOrderModal",
  props: {
    medication: {}
  },
  setup(__props) {
    ...
    const { voidOrderFormData, processVoiding, trashOutline } = useVoidDrugOrder(props.medication);
    const handleSubmission = async () => {
      if (formRef.value?.validateForm() == null) {
        const data = formRef.value?.getFormValues();
        if (!data) return;
        const msg = `Are you sure you want to void ${props.medication?.drug_name}?`;
        if (!await alertConfirmation(msg)) return;
        try {
          await processVoiding(data);
          toastSuccess("Medication marked for voiding");
          modalController.dismiss({ voided: true, order_id: props.medication.order_id });
        } catch (e) {
          toastWarning(`${e}`);
        }
      }
    ...
    return (_ctx, _cache) => {
      return openBlock(), createBlock(StandardModal, {
        title: "Void Medication",
        subtitle: __props.medication?.drug_name,
        headerIcon: unref(trashOutline)
      }, {
        "top-buttons": withCtx(() => [
        ...
        content: withCtx(() => [
          createBaseVNode("div", _hoisted_1$1, [
            createVNode(StandardForm, {
              formData: unref(voidOrderFormData),
              ref_key: "formRef",
              ref: formRef
            }, null, 8, ["formData"])
          ])
...
const _hoisted_13 = { class: "text-subtle" };
const _hoisted_14 = {
  key: 0,
  class: "error-text"
};
const _hoisted_15 = { class: "text-right" };
const _hoisted_16 = { class: "flex justify-end items-center gap-2" };
const _hoisted_17 = {
  key: 2,
  style: { "margin-left": "10px", "margin-right": "10px" },
  class: "p-4"
};
const _hoisted_18 = { class: "program-title" };
const _hoisted_19 = { class: "group-header-box" };
const _hoisted_20 = { class: "date-label" };
const _hoisted_21 = {
  key: 0,
  class: "diag-badge-row"
};
...
const _hoisted_23 = { style: { "position": "absolute", "top": "-5px", "right": "-5px" } };
const _hoisted_24 = { class: "card-med-title" };
const _hoisted_25 = { style: { "padding-right": "25px" } };
const _hoisted_26 = { class: "card-details" };
const _hoisted_27 = { class: "text-subtle" };
const _hoisted_28 = { class: "text-subtle" };
const _hoisted_29 = { class: "text-subtle" };
const _hoisted_30 = {
  key: 0,
  class: "error-text"
};
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const voidedOrderIds = computed(() => {
      const voidedData = patient.value?.voidedDrugOders;
      if (!voidedData) return [];
      const savedIds = (voidedData.saved || []).map((o) => o.order_id);
      const unsavedIds = (voidedData.unsaved || []).flatMap(
        (entry) => (entry.voidedDrugOders || []).map((v) => v.drug_order_id)
      );
      return [.../* @__PURE__ */ new Set([...savedIds, ...unsavedIds])];
    });
    async function openVoidModal(medication) {
      console.log("Opening void modal for medication:", medication);
    ...
    const closeVisitRedirection = async (patientData) => {
      const Confirm = await alertConfirmation("Are you sure you want to close the visit?");
      if (Confirm) {
    ...
    const getMedicationDuration = (medication) => {
      const endDateStr = medication.auto_expire_date || medication.expire_date;
      if (!medication.start_date || !endDateStr) return "N/A";
      const start = new Date(medication.start_date);
      const end = new Date(endDateStr);
      const diffMs = end.getTime() - start.getTime();
      const days = Math.round(diffMs / (1e3 * 60 * 60 * 24));
    ...
    const groupMedicationsByProgram = async (medicationsList) => {
      ...
      const activeMedications = medicationsList.filter((medication) => {
        return !voidedOrderIds.value.includes(medication.order_id);
      });
      ...
      const allHaveQuantity = activeMedications.every((med) => med.quantity);
      if (allHaveQuantity && !isInitialLoad.value) {
        if (Service.getProgramID() == 14) {
          const Confirm = await alertConfirmation("All medications dispensed. Would you like to print the visit summary before closing?");
          if (Confirm) {
                      ...
                      (openBlock(true), createElementBlock(Fragment, null, renderList(groupMedicationsByDate(medications2), (medicationGroup, date) => {
                        return openBlock(), createElementBlock(Fragment, { key: date }, [
                          createBaseVNode("div", _hoisted_5, [
                            ...
                            unref(diagnosesByDate)[`${date}_${medications2[0]?.program_id}`] ? (openBlock(), createElementBlock("div", _hoisted_8, [
                              (openBlock(true), createElementBlock(Fragment, null, renderList(unref(diagnosesByDate)[`${date}_${medications2[0]?.program_id}`], (diag) => {
                              ...
                              createBaseVNode("table", _hoisted_9, [
                                _cache[4] || (_cache[4] = createBaseVNode("thead", null, [
                                  createBaseVNode("tr", null, [
                                    ...
                                    createBaseVNode("th", { style: { "width": "15%" } }, "Duration"),
                                ...
                                createBaseVNode("tbody", null, [
                                  (openBlock(true), createElementBlock(Fragment, null, renderList(medicationGroup, (medication) => {
                                    return openBlock(), createElementBlock("tr", {
                                      ...
                                      createBaseVNode("td", null, [
                                        createBaseVNode("span", _hoisted_13, toDisplayString(getMedicationDuration(medication)), 1)
                                      ]),
                                      createBaseVNode("td", null, [
                                      ...
                                      createBaseVNode("td", _hoisted_15, [
                                        createBaseVNode("div", _hoisted_16, [
                                          ...
                                          createVNode(unref(IonButton), {
                                            ...
                                            fill: "clear",
                                            size: "small",
                                            onClick: ($event) => openVoidModal(medication)
                                          }, {
                                            default: withCtx(() => [
                                              createVNode(unref(IonIcon), {
                      ...
                      (openBlock(true), createElementBlock(Fragment, null, renderList(groupMedicationsByDate(medications2), (medicationGroup, date) => {
                        return openBlock(), createElementBlock(Fragment, { key: date }, [
                          createBaseVNode("div", _hoisted_19, [
                            ...
                            unref(diagnosesByDate)[`${date}_${medicationGroup[0]?.program_id}`] ? (openBlock(), createElementBlock("div", _hoisted_21, [
                              (openBlock(true), createElementBlock(Fragment, null, renderList(unref(diagnosesByDate)[`${date}_${medicationGroup[0]?.program_id}`], (diag) => {
                                return openBlock(), createElementBlock("span", {
                                  key: diag.concept_id,
                                  class: "diag-mini-pill"
                                }, toDisplayString(diag.resolvedName), 1);
                                      ...
                                      default: withCtx(() => [
                                        createBaseVNode("div", _hoisted_22, [
                                          ...
                                          createBaseVNode("div", _hoisted_24, [
                                            ...
                                            createBaseVNode("span", _hoisted_25, toDisplayString(medication.drug_name), 1)
                                          ]),
                                          createBaseVNode("div", _hoisted_26, [
                                            createBaseVNode("p", _hoisted_27, toDisplayString(getFrequencyLabel(medication.frequency)), 1),
                                            createBaseVNode("p", _hoisted_28, toDisplayString(medication.dose) + " " + toDisplayString(medication.units), 1),
                                            createBaseVNode("p", _hoisted_29, toDisplayString(getMedicationDuration(medication)), 1),
                                            createVNode(unref(IonInput), {
                                              type: "number",
                                              placeholder: "Qty",
                                              modelValue: medication.amountToDispense,
                                            ...
                                            medication.error ? (openBlock(), createElementBlock("div", _hoisted_30, toDisplayString(medication.error), 1)) : createCommentVNode("", true)
                                          ]),
                                          createVNode(unref(IonButton), {
                                            expand: "block",
                                            color: "success",
...
const NCDMedicationDispenstion = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-c416a584"]]);
