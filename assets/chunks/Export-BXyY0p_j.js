import { el as s, w as watch, f as ref, cA as E, cB as autoTable, em as writeBlob, en as Directory, eo as Filesystem, ep as Encoding } from './vendor-DvGYa1v4.js';
import { l as lodashExports } from './lodash-CMC_Grj0.js';
import { t as toastWarning, x as toastDanger, G as toastSuccess } from '../index-DoXuJQPv.js';
...
function usePlatform() {
  ...
  if (typeof configuredProfile === "string" && `${configuredProfile}`.match(/\{.*\}/)) {
    ...
    if (!lodashExports.isEmpty(platformProfiles.value)) {
      if (s("mobile") && "Mobile" in platformProfiles.value) {
        activePlatformProfile.value = platformProfiles.value["Mobile"];
      } else if (s("desktop") && "Desktop" in platformProfiles.value) {
