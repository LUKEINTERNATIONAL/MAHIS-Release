import { c as computed, s as defineComponent, w as watch, a2 as onMounted, n as nextTick, y as openBlock, z as createElementBlock, A as createVNode, B as withCtx, F as unref, bd as IonCardContent, bK as IonCard, f as ref, O as createBlock, C as createBaseVNode, J as Fragment, R as renderList, D as toDisplayString, H as createCommentVNode, bA as createStaticVNode, Q as alertCircleOutline, L as IonIcon, a5 as createTextVNode, a4 as normalizeClass, eI as shieldCheckmarkOutline, aS as medicalOutline, G as closeCircleOutline, eJ as helpCircleOutline, c3 as checkmarkCircleOutline, eK as lockClosedOutline, b8 as calendarOutline, e3 as createOutline, eL as sort } from './vendor-CjPalcaW.js';
import { y as StandardValidations, u as useDemographicsStore, z as StandardForm, K as ObservationService, b as EncounterTypeId, t as toastWarning, G as toastSuccess, _ as _export_sfc, aq as ConceptService, n as icons } from '../index-CF4kq0Mc.js';
...
const LabourAllergies = /* @__PURE__ */ _export_sfc(_sfc_main$5, [["__scopeId", "data-v-a0abcd72"]]);
...
const _hoisted_2$3 = { class: "anc-list" };
const _hoisted_3$3 = {
  key: 0,
...
const _sfc_main$4 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    const demographicsStore = useDemographicsStore();
    const { patient } = storeToRefs(demographicsStore);
    const ancPastSurgeries = ref([]);
    const showAll = ref(false);
    const displayedPastSurgeries = computed(() => {
      if (showAll.value || ancPastSurgeries.value.length <= 3) {
        return ancPastSurgeries.value;
      }
      return ancPastSurgeries.value.slice(0, 3);
    });
    const toggleSeeMore = () => {
      showAll.value = !showAll.value;
    };
    async function loadAncPastSurgeries() {
      if (!patient.value?.patientID) return;
      try {
        const encounters = await ObservationService.getObsByEncounterId(EncounterTypeId.MEDICAL_HISTORY);
        const allObs = (encounters || []).flatMap((encounter) => encounter?.obs || []);
        const pastSurgeriesConceptNames = ["past surgeries", "Other surgery notes"];
        const surgeryObs = allObs.filter((o) => o?.concept_name && pastSurgeriesConceptNames.includes(String(o.concept_name).trim()));
        if (!surgeryObs.length) {
          ancPastSurgeries.value = [];
          return;
        }
        const displayValues = [];
        for (const o of surgeryObs) {
          let label;
          if (o?.value_text) {
            label = formatSurgeryLabel(String(o.value_text).trim());
          } else if (o?.value_coded) {
            label = await ConceptService.getConceptName(o.value_coded);
            if (label) label = formatSurgeryLabel(label);
          } else {
            continue;
          }
          if (label && label.toLowerCase() !== "none") displayValues.push(label);
        }
        const otherNotesObs = allObs.find((o) => o?.concept_name && String(o.concept_name).trim() === "Other surgery notes" && o?.value_text);
        const otherNotes = otherNotesObs ? String(otherNotesObs.value_text).trim() : "";
        if (otherNotes && displayValues.some((v) => v.toLowerCase().includes("other"))) {
          const otherIdx = displayValues.findIndex((v) => v.toLowerCase().includes("other"));
          if (otherIdx !== -1) displayValues[otherIdx] = `Other: ${otherNotes}`;
        }
        ancPastSurgeries.value = Array.from(new Set(displayValues));
      } catch (error) {
        console.error("Error loading ANC past surgeries:", error);
        ancPastSurgeries.value = [];
      }
    }
    function formatSurgeryLabel(value) {
      if (!value) return value;
      return value.split(/\s+/).map((w) => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(" ");
    }
    watch(
      () => patient.value?.patientID,
      async (newID, oldID) => {
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("div", _hoisted_1$4, [
        ancPastSurgeries.value.length ? (openBlock(), createBlock(unref(IonCard), {
          key: 0,
          class: "section"
        }, {
          default: withCtx(() => [
            createVNode(unref(IonCardContent), null, {
              default: withCtx(() => [
                _cache[0] || (_cache[0] = createBaseVNode("div", { class: "sub_item_header dashed_bottom_border" }, "Existing past surgeries (ANC)", -1)),
                createBaseVNode("div", _hoisted_2$3, [
                  (openBlock(true), createElementBlock(Fragment, null, renderList(displayedPastSurgeries.value, (surgery, idx) => {
                    return openBlock(), createElementBlock("div", {
                      key: `${surgery}-${idx}`,
                      class: "anc-item"
                    }, toDisplayString(surgery), 1);
                  }), 128))
                ]),
                ancPastSurgeries.value.length > 3 ? (openBlock(), createElementBlock("div", _hoisted_3$3, [
...
const LabourPastSurgeries = /* @__PURE__ */ _export_sfc(_sfc_main$4, [["__scopeId", "data-v-091ff254"]]);
...
const _sfc_main$3 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    const onSubmit = async () => {
      ...
      const data = formRef.value?.getFormValues();
      if (Object.keys(data || {}).length === 0) return;
      if (await ObservationService.buildSaveObs(data, EncounterTypeId.LABOUR_ASSESSMENT)) toastSuccess("Chronic Health Conditions data saved successfully");
      formRef.value?.resetForm();
    };
    __expose({
      onSubmit,
...
const LabourChronicHealthConditions = /* @__PURE__ */ _export_sfc(_sfc_main$3, [["__scopeId", "data-v-50231778"]]);
...
const _hoisted_3$1 = { class: "status-selection" };
  ...
  setup(__props, { expose: __expose }) {
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("div", _hoisted_1$2, [
        _cache[11] || (_cache[11] = createStaticVNode('<div class="dashed-separator" data-v-12f16445></div><div class="field-header-container" data-v-12f16445><div class="header-with-icon" data-v-12f16445><div data-v-12f16445><h6 class="field-header" data-v-12f16445>Select Tetanus Immunization Status <span class="required-asterisk" data-v-12f16445>*</span></h6></div></div></div>', 2)),
...
const LabourVaccineHistory = /* @__PURE__ */ _export_sfc(_sfc_main$2, [["__scopeId", "data-v-12f16445"]]);
...
const LabourMedications = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["__scopeId", "data-v-e174f53b"]]);
...
const LabourPastPregnancyComplications = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-906e1b84"]]);
