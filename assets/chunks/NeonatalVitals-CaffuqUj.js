import { s as defineComponent, h as inject, cm as onBeforeUnmount, w as watch, y as openBlock, z as createElementBlock, C as createBaseVNode, D as toDisplayString, A as createVNode, F as unref, f as ref, c as computed, a3 as onMounted, n as nextTick, aI as IonContent, bx as IonPage, aN as useRouter, f3 as provide, x as resolveComponent, O as createBlock, B as withCtx } from './vendor-QOXikrLM.js';
import { s as storeToRefs } from './pinia-BRP2SQHv.js';
import { z as StandardForm, _ as _export_sfc, u as useDemographicsStore, T as Toolbar, t as toastWarning, G as toastSuccess } from '../index-SKNIAGXC.js';
import { z as neonatalVitalsSections, A as neonatalVitalsFormKey, N as NeonatalStepper, B as useNeonatalVitalsStore } from './NeonatalStepper-DJUdVdam.js';
import { r as resolveNeonatalBirthWeightInGrams } from './neonatalBirthWeightPrefill-BxG_la5K.js';
import { T as TriageConfirmDialog } from './TriageConfirmDialog-BlMzR2-I.js';
import { u as useNeonatalTriageStore } from './triage-CxFMIncp.js';
...
const _sfc_main$5 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { expose: __expose }) {
    ...
    const enrichedFormData = computed(() => {
      return baseFormData.value.map((field) => {
    ...
    async function prefillBirthWeightFromTriage() {
      ...
      try {
        const grams = await resolveNeonatalBirthWeightInGrams(patientId, patient.value);
        await nextTick();
        if (grams != null) {
          formRef.value.setFormValue("birthWeight", String(Math.round(grams)));
        }
      } catch (error) {
        console.error("Failed to prefill birth weight for vitals:", error);
      } finally {
    ...
    __expose({
      getFormRef: () => formRef.value,
      validateForm: () => formRef.value?.validateForm?.() || null,
      /** Omit display-only `birthWeight` so NeonatalStepper does not merge it into vitals Pinia state / save payload. */
      getFormValues: () => {
        const raw = formRef.value?.getFormValues?.() || {};
        const { birthWeight: _omit, ...rest } = raw;
        return rest;
      }
    });
    return (_ctx, _cache) => {
...
const WeightMeasurementsSection = /* @__PURE__ */ _export_sfc(_sfc_main$5, [["__scopeId", "data-v-520eb02e"]]);
...
const BloodSugarSection = /* @__PURE__ */ _export_sfc(_sfc_main$4, [["__scopeId", "data-v-f5d92e16"]]);
...
const _sfc_main = defineComponent({
  ...
  setup() {
    ...
    const syncForPatient = () => {
      vitalsStore.initializeForRawPatientId(patient.value?.patientID);
    };
    syncForPatient();
    watch(
      () => patient.value?.patientID,
    ...
    const getSaveFunction = (currentIndex) => {
      if (currentIndex === stepperData.length - 1) {
        return async () => {
          ...
          try {
            const saveResult = await vitalsStore.saveAssessment(patientId);
            if (!saveResult.ok) {
              if (saveResult.reason === "saving") return;
              if (saveResult.reason === "no_data") {
                toastWarning("No valid vital signs to save");
                return;
              }
...
const NeonatalVitals = /* @__PURE__ */ _export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-67be677c"]]);
