...
import { S as Service, b3 as useTreatmentPlanStore, u as useDemographicsStore, G as toastSuccess, t as toastWarning, bf as useNCDMedicationsStore, H as HisDate, a7 as useUserStore, K as ObservationService, b as EncounterTypeId } from '../index-BQBxfUAX.js';
  ...
  try {
    ...
    if (drugOrders.length > 0) {
      ...
      const existingMap = new Map(
        patientData.MedicationOrder.unsaved.map((obj) => [`${obj.drug_inventory_id}-${obj.start_date}`, obj])
      );
      drugOrders.forEach((order) => {
        existingMap.set(`${order.drug_inventory_id}-${order.start_date}`, order);
      });
      patientData.MedicationOrder.unsaved = Array.from(existingMap.values());
      toastSuccess("Medication orders staged successfully");
      return patientData;
    }
    return patientData;
  } catch (error) {
    toastWarning("Unable to stage medication orders!");
  }
}
